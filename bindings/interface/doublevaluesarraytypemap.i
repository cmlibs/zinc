...
%typemap(in) (const double *valuesIn4)
{
	/* Applying in array of size 4 typemap */
	if (PyList_Check($input) && PyList_Size($input) == 4)
	{
		$1 = new double[4];
		for (int i = 0; i < 4; i++)
		{
	...
	{
		PyErr_SetString(PyExc_TypeError,"not a list of size 4");
		return NULL;
	}
};

%typemap(freearg) (double const *valuesIn4)
{
	delete[] $1;
};
...
%typemap(argout) (double *valuesOut4)
{
	/* Applying out array of size 4 typemap */
	PyObject *o;
	o = PyList_New(4);
	for (int i = 0 ; i < 4; i++)
	{
