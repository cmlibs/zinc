...
FUNCTION( PREPEND_TO_EACH_ITEM_IN_LIST PREPEND_THIS ITEM_LIST OUT_ITEM_LIST )
	...
	FILE( STRINGS ${CMAKE_SOURCE_DIR}/source/wx_non_strict_c.filenames C_NON_STRICT_FILES )
	FILE( STRINGS ${CMAKE_SOURCE_DIR}/source/wx_non_strict_cpp.filenames CPP_NON_STRICT_FILES )
	FILE( STRINGS ${CMAKE_SOURCE_DIR}/source/wx_not_initialised.filenames NOT_INITIALISED_FILES )

	FOREACH( ITEM ${ITEM_LIST} )
		SET( SUPPRESS_WARNINGS )
		LIST( FIND C_NON_STRICT_FILES ${ITEM} INDEX_OF_SRC )
		IF( ${INDEX_OF_SRC} GREATER -1 )
			#MESSAGE( STATUS "C: ${PREPEND_THIS}${ITEM} -Wno-parentheses -Wno-switch" )
			SET( SUPPRESS_WARNINGS "${SUPPRESS_WARNINGS} -Wno-parentheses -Wno-switch" )
		ENDIF( ${INDEX_OF_SRC} GREATER -1 )
		LIST( FIND CPP_NON_STRICT_FILES ${ITEM} INDEX_OF_SRC )
		IF( ${INDEX_OF_SRC} GREATER -1 )
			#MESSAGE( STATUS "CXX: ${PREPEND_THIS}${ITEM} -Wno-parentheses -Wno-switch -Wno-unused-parameter" )
			SET( SUPPRESS_WARNINGS "${SUPPRESS_WARNINGS} -Wno-parentheses -Wno-switch" )
		ENDIF( ${INDEX_OF_SRC} GREATER -1 )
		LIST( FIND NOT_INITIALISED_FILES ${ITEM} INDEX_OF_SRC )
		IF( ${INDEX_OF_SRC} GREATER -1 )
			#MESSAGE( STATUS "NOT INIT: ${PREPEND_THIS}${ITEM}" )
			SET( SUPPRESS_WARNINGS "${SUPPRESS_WARNINGS} -Wno-uninitialized" )
		ENDIF( ${INDEX_OF_SRC} GREATER -1 )
		IF( SUPPRESS_WARNINGS )
			SET_SOURCE_FILES_PROPERTIES( "${PREPEND_THIS}${ITEM}" PROPERTIES COMPILE_FLAGS ${SUPPRESS_WARNINGS} )
