...
struct GT_surface *create_GT_surface_streamribbon_FE_element(
	...
	struct FE_region *fe_region, enum Cmiss_graphic_polygon_render_mode polygon_render_mode)
{
							...
							(0.0<length)&&((data_type!=STREAM_FIELD_SCALAR) || data_field))
		{
				...
				&stream_vectors,&stream_normals,&stream_data, fe_region))
			{
				if (0<number_of_stream_points)
				{
					...
					{
						if (NULL != (surface=CREATE(GT_surface)(g_SHADED_TEXMAP,polygon_render_mode,
