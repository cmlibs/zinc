...
{
	if (image && minimum && maximum)
	{
		*minimum = 0;
		*maximum = 0;
#if defined (USE_IMAGEMAGICK)
		if (image->magick_image)
		{
			if (image->number_of_components > 0)
			{
				ExceptionInfo magick_exception;
				GetExceptionInfo(&magick_exception);

				Image *temp_magick_image = image->magick_image;
				if (image->number_of_components <= 2)
				{
					while (temp_magick_image)
					{
						double localMinimum = 0, localMaximum = 0;
						GetImageChannelRange(temp_magick_image, GrayChannel, &localMinimum,
							&localMaximum, &magick_exception);
				...
				else
				{
					...
					{
						double localMinimum = 0, localMaximum = 0;
						GetImageChannelRange(temp_magick_image, RedChannel, &localMinimum,
							&localMaximum, &magick_exception);
						if (*minimum > localMinimum)
						{
							*minimum = localMinimum;
						...
						GetImageChannelRange(temp_magick_image, GreenChannel, &localMinimum,
							&localMaximum, &magick_exception);
						if (*minimum > localMinimum)
						{
							*minimum = localMinimum;
						...
						GetImageChannelRange(temp_magick_image, BlueChannel, &localMinimum,
							&localMaximum, &magick_exception);
						if (*minimum > localMinimum)
						{
							*minimum = localMinimum;
