...
$(SOURCE_PATH)/unemap_sgi3d_optimised.make : $(SOURCE_PATH)/unemap.imake $(SOURCE_PATH)/common.imake unemap.make
	$(COMMON_IMAKE_RULE) \
...
$(SOURCE_PATH)/unemap_linux_optimised.make : $(SOURCE_PATH)/unemap.imake $(SOURCE_PATH)/common.imake unemap.make
	$(COMMON_IMAKE_RULE) \
...
$(SOURCE_PATH)/unemap_linux_3d_optimised.make : $(SOURCE_PATH)/unemap.imake $(SOURCE_PATH)/common.imake unemap.make
	$(COMMON_IMAKE_RULE) \
...
update :
	if ( [ "$(PWD)" -ef "$(PRODUCT_PATH)" ] && [ "$(USER)" = "cmiss" ] ); then \
		...
		cd $(PRODUCT_PATH) && \
		$(MAKE) -f unemap.make unemap32 unemap_nodes unemap_3d unemap_3d_optimised unemap_optimised unemap_64 unemap_memorycheck utilities; \
		ssh 130.216.208.156 'setenv CMISS_ROOT /product/cmiss ; cd $(PRODUCT_PATH) ; $(MAKE) -f unemap.make unemap_linux unemap_linux_nodes unemap_linux_3d unemap_linux_optimised unemap_linux_3d_optimised utilities_linux' ; \
		cd $(PRODUCT_SOURCE_PATH) && \
		chgrp -R cmgui_programmers *; \
	else \
