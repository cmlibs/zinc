...
TEST(cmzn_spectrum_api, valid_args)
{
	...
	EXPECT_EQ(CMZN_OK, result = cmzn_spectrum_component_set_range_maximum(component1, 20.0));

	double double_result = cmzn_spectrum_component_get_range_maximum(component1);
	EXPECT_EQ(20.0, double_result);

	EXPECT_EQ(CMZN_OK, result = cmzn_spectrum_component_set_active(component1, false));

	bool bool_result = cmzn_spectrum_component_is_active(component1);
	EXPECT_FALSE(bool_result);

	EXPECT_EQ(CMZN_OK, result = cmzn_spectrum_component_set_colour_reverse(component1, true));

	bool_result = cmzn_spectrum_component_is_colour_reverse(component1);
	EXPECT_TRUE(bool_result);

	EXPECT_EQ(CMZN_OK, result = cmzn_spectrum_component_set_extend_above(
		component1, false));

	bool_result = cmzn_spectrum_component_is_extend_above(component1);
	EXPECT_FALSE(bool_result);

	EXPECT_EQ(CMZN_OK, result = cmzn_spectrum_component_set_extend_below(component1, false));
