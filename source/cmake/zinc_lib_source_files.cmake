
# Defines ZINC_LIB_SRCS
SET( ZINC_LIB_BASE_SRCS  ${COMFILE_SRCS} ${COMMAND_SRCS} ${CONTEXT_SRCS} ${GENERAL_SRCS}
	${COMPUTED_FIELD_SRCS} ${CURVE_SRCS} ${ELEMENT_SRCS} ${EMOTER_SRCS}
	${FIELD_IO_SRCS} ${FINITE_ELEMENT_GRAPHICS_SRCS} ${FINITE_ELEMENT_ADDITIONAL_SRCS}
	${GRAPHICS_SRCS} ${IMAGE_PROCESSING_SRCS} ${INTERACTION_SRCS}
	${IO_DEVICES_SRCS} ${MESH_SRCS} ${NODE_SRCS} ${SELECTION_SRCS} ${INTERPRETER_SRCS} 
	${THREE_D_DRAWING_SRCS} ${TIME_SRCS} ${USER_INTERFACE_SRCS} ${OPENCASCADE_SRCS} 
	${FINITE_ELEMENT_CORE_SRCS} ${REGION_SRCS} )
SET( ZINC_LIB_BASE_HDRS ${API_HDRS} ${COMFILE_HDRS} ${COMMAND_HDRS} ${CONTEXT_HDRS}
	${COMPUTED_FIELD_HDRS} ${CURVE_HDRS} ${ELEMENT_HDRS} ${EMOTER_HDRS} ${GENERAL_HDRS}
	${FIELD_IO_HDRS} ${FINITE_ELEMENT_GRAPHICS_HDRS} ${FINITE_ELEMENT_ADDITIONAL_HDRS}
	${GRAPHICS_HDRS} ${IMAGE_PROCESSING_HDRS} ${INTERACTION_HDRS}
	${IO_DEVICES_HDRS} ${MESH_HDRS} ${NODE_HDRS} ${SELECTION_HDRS} ${INTERPRETER_HDRS} 
	${THREE_D_DRAWING_HDRS} ${TIME_HDRS} ${USER_INTERFACE_HDRS} ${OPENCASCADE_HDRS} 
	${FINITE_ELEMENT_CORE_HDRS} ${REGION_HDRS} )

IF( WX_USER_INTERFACE )
	SET( ZINC_LIB_INTERFACE_SRCS ${API_INTERFACE_SRCS} ${COLOUR_INTERFACE_SRCS}
		${COMFILE_INTERFACE_SRCS}
		${COMMAND_INTERFACE_SRCS} ${COMPUTED_FIELD_INTERFACE_SRCS}
		${GRAPHICS_INTERFACE_SRCS}
		${MATERIAL_INTERFACE_SRCS} ${NODE_INTERFACE_SRCS}
		${REGION_INTERFACE_SRCS}
		${TRANSFORMATION_INTERFACE_SRCS} )
	SET( ZINC_LIB_INTERFACE_HDRS ${API_INTERFACE_HDRS} ${COLOUR_INTERFACE_HDRS}
		${COMFILE_INTERFACE_HDRS}
		${COMMAND_INTERFACE_HDRS} ${COMPUTED_FIELD_INTERFACE_HDRS}
		${GRAPHICS_INTERFACE_HDRS}
		${MATERIAL_INTERFACE_HDRS} ${NODE_INTERFACE_HDRS}
		${REGION_INTERFACE_HDRS}
		${TRANSFORMATION_INTERFACE_HDRS} )
ENDIF( WX_USER_INTERFACE )

IF( WIN32_USER_INTERFACE )
	SET( ZINC_LIB_INTERFACE_SRCS ${API_INTERFACE_SRCS} 
		${COMMAND_INTERFACE_SRCS}
		${COMPUTED_FIELD_INTERFACE_SRCS}
		${GRAPHICS_INTERFACE_SRCS} )
	SET( ZINC_LIB_INTERFACE_HDRS ${API_INTERFACE_HDRS}
		${COMMAND_INTERFACE_HDRS} ${COMPUTED_FIELD_INTERFACE_HDRS}
		${GRAPHICS_INTERFACE_HDRS} )
ENDIF( WIN32_USER_INTERFACE )

IF( GTK_USER_INTERFACE )
    SET( ZINC_LIB_INTERFACE_SRCS ${API_INTERFACE_SRCS}
        ${COMMAND_INTERFACE_SRCS}
        ${COMPUTED_FIELD_INTERFACE_SRCS}
        ${GRAPHICS_INTERFACE_SRCS} )
    SET( ZINC_LIB_INTERFACE_HDRS ${API_INTERFACE_HDRS}
        ${COMMAND_INTERFACE_HDRS} ${COMPUTED_FIELD_INTERFACE_HDRS}
        ${GRAPHICS_INTERFACE_HDRS} )
ENDIF( GTK_USER_INTERFACE ) 

SET( ZINC_LIB_SRCS
	${ZINC_LIB_BASE_SRCS}
	${ZINC_LIB_INTERFACE_SRCS} )

SET( ZINC_LIB_HDRS
	${ZINC_LIB_BASE_HDRS}
	${ZINC_LIB_INTERFACE_HDRS}
	${wxWidgets_GENERATED_HDRS} )
