		...
		else // (parent_region == region)
		{
			...
			if (pos == sub_group_map.end())
			{
				...
				if (generic_field)
				{
					...
					// Not calling Cmiss_field_set_attribute_integer(sub_group, CMISS_FIELD_ATTRIBUTE_IS_MANAGED, 0);
	...
	if (!contains_all)
	{
		if (!local_element_group && mesh)
		{
			...
			if (!local_element_group)
			{
				...
				Cmiss_field_set_attribute_integer(local_element_group, CMISS_FIELD_ATTRIBUTE_IS_MANAGED, 0);
