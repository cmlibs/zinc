...
char computed_field_quaternions_type_string[] = "quaternions";

class Computed_field_quaternions : public Computed_field_core
{
public:
...
private:
	...
	char *get_type_string()
	{
		return(computed_field_quaternions_type_string);
	}

	int compare(Computed_field_core* other_field)
	 {
		if (dynamic_cast<Computed_field_quaternions*>(other_field))
		{
			return 1;
		}
		else
...
Computed_field_quaternions::~Computed_field_quaternions()
/*******************************************************************************
...
==============================================================================*/
{
	ENTER(Computed_field_quaternions::~Computed_field_quaternions);
	if (field)
	{
		 if (m)
		{
...
int Computed_field_quaternions::clear_cache()
/*******************************************************************************
LAST MODIFIED : 4 October 2007
...
==============================================================================*/
{
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_quaternions::clear_cache.  "
			"Invalid argument(s)");
	}
	LEAVE;
...
int Computed_field_quaternions::evaluate()
/*******************************************************************************
...
==============================================================================*/
{
	 ...
	 double wx, wy, wz, xx, yy, yz, xy, xz, zz, x2, y2, z2, 
		 	x_translate, y_translate, z_translate, magnitude, 
	...
	if (field)
	{
		 ...
		 y = y / magnitude;
		 z = z / magnitude;
		 x_translate = (double)(field->source_fields[0]->values[4]);
		 y_translate = (double)(field->source_fields[0]->values[5]);
		 z_translate = (double)(field->source_fields[0]->values[6]);
		 // convert the final quaternion to transformation matrix
		 x2 = x + x;
		 y2 = y + y;
		 z2 = z + z;
		 xx = x * x2;
		 ...
		 {
				display_message(ERROR_MESSAGE,
					 "Computed_field_quaternions::evaluate.  "
...
int Computed_field_quaternions::evaluate_cache_at_location(
	 Field_location* location)
/*******************************************************************************
LAST MODIFIED : 5 October 2007
...
int Computed_field_quaternions::list()
/*******************************************************************************
LAST MODIFIED : 25 August 2006
...
==============================================================================*/
{
	...
	ENTER(List_Computed_field_quaternions);
	if (field)
	{
		display_message(INFORMATION_MESSAGE,
			"    quaternions field : %s\n",field->source_fields[0]->name);
	...
	{
		display_message(ERROR_MESSAGE,
			"list_Computed_field_quaternions.  Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;
...
char *Computed_field_quaternions::get_command_string()
/*******************************************************************************
LAST MODIFIED : 5 October 2007
...
==============================================================================*/
{
	...
	ENTER(Computed_field_quaternions::get_command_string);
	command_string = (char *)NULL;
	if (field)
	{
		error = 0;
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_quaternions::get_command_string.  "
			"Invalid field");
	}
	LEAVE;
...
DESCRIPTION :
Converts <field> to type 'quaternions' which performs a full conversion on
the matrix in <source_field> and returns the transformation matrix, where
<source_field> must have 7 components.
If function fails, field is guaranteed to be unchanged from its original state,
although its cache may be lost.
==============================================================================*/
{
	...
	if (field && source_field)
	{
		if (source_field->number_of_components == 7)
		{
			/* 1. make dynamic allocations for any new type-specific data */
			number_of_source_fields = 1;
			if (ALLOCATE(source_fields,struct Computed_field *,
			...
			{
				display_message(ERROR_MESSAGE,
					"Computed_field_set_type_quaternions.  Not enough memory");
				DEALLOCATE(source_fields);
				return_code=0;
			}
		}
		...
		{
			display_message(ERROR_MESSAGE,"Computed_field_set_type_quaternions.  "
				"Field %s does not have 7 components",source_field->name);
			return_code = 0;
		}
	}
	else
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_set_type_quaternions.  Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;
...
int Computed_field_get_type_quaternions(struct Computed_field *field,
	struct Computed_field **source_field)
/*******************************************************************************
LAST MODIFIED : 25 August 2006
...
==============================================================================*/
{
	...
	ENTER(Computed_field_get_type_quatenions);
	if (field && (dynamic_cast<Computed_field_quaternions*>(field->core)) &&
		source_field)
	{
		*source_field = field->source_fields[0];
		return_code = 1;
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_get_type_quaternions.  Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;

	return (return_code);
} /* Computed_field_get_type_quaternions */

int define_Computed_field_type_quaternions(struct Parse_state *state,
	void *field_void, void *computed_field_matrix_operations_package_void)
/*******************************************************************************
LAST MODIFIED : 25 August 2006
...
==============================================================================*/
{
			...
			computed_field_matrix_operations_package_void))
	{
		return_code=1;
		source_field = (struct Computed_field *)NULL;
		if (computed_field_quaternions_type_string ==
			Computed_field_get_type_string(field))
		{
			return_code = Computed_field_get_type_quaternions(field, &source_field);
		}
		if (return_code)
		{
			if (source_field)
			...
			option_table = CREATE(Option_table)();
			Option_table_add_help(option_table,
				 "A quaternions field rotates a given coordiate field. "
				 "The components of the quaternions field are expected to "
				 "the w, x, y, z components of a quaternions and the x, y "
				 "and z translation.of a 4x4 transformation matrix."
				 "(7 components in total). gfx define transformation..."
				 "should be followed after setting up this field");
			set_source_field_data.conditional_function =
				 Computed_field_has_7_components;
			set_source_field_data.conditional_function_user_data = (void *)NULL;
			set_source_field_data.computed_field_manager =
				 computed_field_matrix_operations_package->computed_field_manager;
			Option_table_add_entry(option_table, "field", &source_field,
	...
	{
		display_message(ERROR_MESSAGE,
			"define_Computed_field_type_quaternions.  Invalid argument(s)");
		return_code=0;
	}
	LEAVE;

	return (return_code);
} /* define_Computed_field_type_quaternions */
...
==============================================================================*/
{
	...
	if (computed_field_package)
	{
		...
		return_code = 
			 Computed_field_package_add_type(computed_field_package,
					computed_field_quaternions_type_string,
					define_Computed_field_type_quaternions,
					computed_field_matrix_operations_package);
	}
	else
