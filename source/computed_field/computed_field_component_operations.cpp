...
==============================================================================*/
{
	...
	ENTER(Computed_field_set_type_log);
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_set_type_log.  Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;

	return (return_code);
} /* Computed_field_set_type_log */
		...
		computed_field_component_operations_package_void))
	{
		...
		if (ALLOCATE(source_fields, struct Computed_field *, 1))
		{
			...
			if (return_code)
			{
				...
				if (return_code)
				{
					return_code = Computed_field_set_type_log(field,
						source_fields[0]);
				}
				if (!return_code)
				{
