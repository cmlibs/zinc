...
/******************************************************************************
NAME:		GetB_TRUEToplevel( wgt )

INPUT:		Widget	wgt		- the top user-created widget 

RETURN:		Widget			- the shell widget

DESCRIPTION:	Returns the B_TRUE toplevel of that interface.
...
-----------------------------------------------------------------------------*/
#ifdef _NO_PROTO
static	Widget 	GetB_TRUEToplevel( wgt )
	Widget 	wgt;
#else
static	Widget 	GetB_TRUEToplevel( Widget wgt )
#endif
{
...
#endif
{
	...
	wcs = (wchar_t *)UxRealloc((void *) wcs,maxlen * sizeof(wchar_t));
	mbstowcs(wcs, value_str, maxlen);
	return(wcs);
}
...
/*???DB.  Is XmGetPixmapByDepth in Motif 1.2 ? */
/*	if (depth)
