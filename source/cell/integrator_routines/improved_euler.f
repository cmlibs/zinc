      SUBROUTINE IMPROVED_EULER(AII,AIO,CONTROL,MODEL,NUM_ODES,NUM_VARS,
...
C###    from t=T to t=Tout given y(i) at t=T using a improved Euler
C###    integration scheme, where NUM_VARS is the total number of
C###    variables. </p><p>
...
C###  See-Also: INTEGRATOR
     ...
     '  YQS(*),T,DT
      ...
      ELSE IF(INTEGER_CONTROL(GET_WORK_SIZES).EQ.1) THEN
        INTEGER_CONTROL(REAL_WORK_SIZE) = 3*NUM_VARS
        INTEGER_CONTROL(INTEGER_WORK_SIZE) = 0
      ELSE
        IF(INTEGER_CONTROL(REAL_WORK_SIZE).GE.(3*NUM_VARS)) THEN
          TIME(TCell) = T
          TIME(DTCell) = DT
          CALL RHSROUTINE(TIME,YQS,REAL_WORK(NUM_VARS+1),CONTROL,MODEL,
     ...
     '      INTEGER_CONTROL(ERROR_CODE))
          ...
          ELSE
            ...
            DO i=1,NUM_ODES
              REAL_WORK(i)=YQS(i)+DT*REAL_WORK(i+NUM_VARS)
            ENDDO !i
            !Copy across any non-ODE variables
     ...
     '        AII,AIO,ARI,ARO,INTEGER_CONTROL(ERROR_CODE))
            ...
            ELSE
              ...
              DO i=1,NUM_ODES
                YQS(i)=YQS(i)+DT*0.5d0*(REAL_WORK(i+NUM_VARS)
     '            +REAL_WORK(i+(2*NUM_VARS)))
              ENDDO !i
            ENDIF !IF(ERR.NE.0)
