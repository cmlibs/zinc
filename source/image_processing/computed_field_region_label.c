...
struct Computed_field_region_label_type_specific_data
{
	float cached_time;
...
static char computed_field_region_label_type_string[] = "region_label";
...
{
	int return_code;
	ENTER(Computed_field_is_type_region_label);
	if (field)
	{
		return_code =
		  (field->type_string == computed_field_region_label_type_string);
	}
	else
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_is_type_region_label.  Missing field");
		return_code = 0;
	}

	return (return_code);
} /* Computed_field_is_type_region_label */

static void Computed_field_region_label_field_change(
...
==============================================================================*/
{
	...
	struct Computed_field_region_label_type_specific_data *data;

	ENTER(Computed_field_region_label_source_field_change);
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_region_label_source_field_change.  "
			"Invalid arguments.");
	}
	LEAVE;
} /* Computed_field_region_label_source_field_change */
...
==============================================================================*/
{
	...
	ENTER(Computed_field_region_label_clear_type_specific);
	if (field && (data =
		(struct Computed_field_region_label_type_specific_data *)
		field->type_specific_data))
	{
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_region_label_clear_type_specific.  "
			"Invalid arguments.");
		return_code = 0;
	}
	LEAVE;
...
==============================================================================*/
{
	struct Computed_field_region_label_type_specific_data *destination,
		*source;

	ENTER(Computed_field_region_label_copy_type_specific);
	if (source_field && destination_field && (source =
		(struct Computed_field_region_label_type_specific_data *)
		source_field->type_specific_data))
	{
		if (ALLOCATE(destination,
			struct Computed_field_region_label_type_specific_data, 1))
		{
			...
			destination->computed_field_manager_callback_id =
				...
				Computed_field_region_label_field_change, (void *)destination_field,
		...
		{
			display_message(ERROR_MESSAGE,
				"Computed_field_region_label_copy_type_specific.  "
...
==============================================================================*/
{
	...
	ENTER(Computed_field_region_label_type_specific_contents_match);
	...
	return (return_code);
} /* Computed_field_region_label_type_specific_contents_match */
...
==============================================================================*/
{
	...
	if (image && (image->dimension == 2) && (image->depth > 0))
	{
		         ...
		         ALLOCATE(data_index1, FE_value, storage_size / image->depth))
		{
			...
			result_index = (FE_value *)storage;
			/* Segment the input image, evaluate a high level to the regions of objects */
			for (i = 0; i < storage_size / image->depth; i++)
			{
			        if ((*data_index) > 0.5)
			        {
				        data_index1[i] = 100000.0;
				}
				else
				{
				        data_index1[i] = 0.0;
			...
			label_index = 0.0;
			max = 0.0;
                        /* Using a clockwise direction to seach the neighbours of each object pixel */
			for (i = 0; i < storage_size / image->depth; i++)
			{
			        if (data_index1[i] == 100000.0)
				{
				        min = 100000.0;
					for (j = 0; j < kernel_size; j++)
					{
					        if((result_index + offsets[j] >= ((FE_value *)storage)) && 
						        (result_index + offsets[j] < ((FE_value *)storage) + storage_size) && (data_index1[i + offsets[j]/image->depth] >= 1.0))
						{
						         min = my_Min(min,data_index1[i + offsets[j]/image->depth]);
						}    
					}
					if (min == 100000.0)
					{
					        label_index += 1.0;
						data_index1[i] = label_index;
					}
					else
					{
					        data_index1[i] = min;
			...
			for (n = 0; n < (image->sizes[0] + image->sizes[1]); n++)
			{
			        for ( i = 0; i < storage_size / image->depth; i++)
				{
					if (data_index1[i] > 0.0)
					{
					        min = 100000.0;
						for (j = 0; j < kernel_size; j++)
						{
			...
			{       
				...
				else
				{
				        for ( i = 0; i < storage_size / image->depth; i++)
					        ...
					        if (data_index1[i] == (FE_value)k)
					        {
						        data_index1[i] = (FE_value)k - (FE_value)l;
					        }
			                }      
				}
			}
			for (i = 0; i < storage_size / image->depth; i++)
			{
			        max = my_Max(max,data_index1[i]);
			}
                        /* printf("number of regions: %f\n",max); */
			for (i = 0; i < storage_size / image->depth; i++)
			{
			        for (k = 0; k < image->depth; k++)
				{
				        result_index[k] = data_index1[i] /max;
				}
				result_index += image->depth;
			}
			if (return_code)
		...
		{
			display_message(ERROR_MESSAGE,
				"Image_cache_region_label.  Not enough memory");
			return_code = 0;
		}
	}
	else
	{
		display_message(ERROR_MESSAGE, "Image_cache_region_label.  "
			"Invalid arguments.");
		return_code=0;
	}
	LEAVE;
...
==============================================================================*/
{
	...
	ENTER(Computed_field_region_label_evaluate_cache_at_node);
	if (field && node &&
		(data = (struct Computed_field_region_label_type_specific_data *)field->type_specific_data))
	{
		return_code = 1;
		...
		if (!data->image->valid)
		{
			...
			return_code = Image_cache_region_label(data->image);
		}
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_region_label_evaluate_cache_at_node.  "
			"Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;
...
static int Computed_field_region_label_evaluate_cache_in_element(
...
==============================================================================*/
{
	...
	ENTER(Computed_field_region_label_evaluate_cache_in_element);
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_region_label_evaluate_cache_in_element.  "
			"Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;
...
int Computed_field_region_label_get_native_resolution(struct Computed_field *field,
...
{       
	...
	ENTER(Computed_field_region_label_get_native_resolution);
	if (field && (data =
		(struct Computed_field_region_label_type_specific_data *)
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_region_label_get_native_resolution.  Missing field");
		return_code=0;
	}

	return (return_code);
} /* Computed_field_region_label_get_native_resolution */

static int list_Computed_field_region_label(
	struct Computed_field *field)
/*******************************************************************************
...
==============================================================================*/
{
	...
	ENTER(List_Computed_field_region_label);
	if (field && (field->type_string==computed_field_region_label_type_string))
	{
	...
	{
		display_message(ERROR_MESSAGE,
			"list_Computed_field_region_label.  Invalid field");
		return_code = 0;
	}
	LEAVE;

	return (return_code);
} /* list_Computed_field_region_label */

static char *Computed_field_region_label_get_command_string(
	struct Computed_field *field)
/*******************************************************************************
LAST MODIFIED : 4 December 2003
...
==============================================================================*/
{
	...
	ENTER(Computed_field_region_label_get_command_string);
	command_string = (char *)NULL;
	if (field&& (field->type_string==computed_field_region_label_type_string)
		&& (data = (struct Computed_field_region_label_type_specific_data *)
		field->type_specific_data) )
	{
		error = 0;
		append_string(&command_string,
			computed_field_region_label_type_string, &error);
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_region_label_get_command_string.  Invalid field");
	}
	LEAVE;

	return (command_string);
} /* Computed_field_region_label_get_command_string */

#define Computed_field_region_label_has_multiple_times \
...
int Computed_field_set_type_region_label(struct Computed_field *field,
...
DESCRIPTION :
Converts <field> to type COMPUTED_FIELD_region_label with the supplied
...
==============================================================================*/
{
	...
	ENTER(Computed_field_set_type_region_label);
		...
		region && graphics_buffer_package)
	{
		...
		data = (struct Computed_field_region_label_type_specific_data *)NULL;
		if (ALLOCATE(source_fields, struct Computed_field *, number_of_source_fields) &&
			ALLOCATE(data, struct Computed_field_region_label_type_specific_data, 1) &&
			...
			Image_cache_update_data_storage(data->image))
		{
			...
			data->computed_field_manager_callback_id =
				MANAGER_REGISTER(Computed_field)(
				Computed_field_region_label_field_change, (void *)field,
				computed_field_manager);

			field->type_specific_data = data;

			/* Set all the methods */
			COMPUTED_FIELD_ESTABLISH_METHODS(region_label);
		}
		else
		{
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_set_type_region_label.  Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;

	return (return_code);
} /* Computed_field_set_type_region_label */

int Computed_field_get_type_region_label(struct Computed_field *field,
	struct Computed_field **source_field,
	struct Computed_field **texture_coordinate_field,
	int *dimension, int **sizes)
/*******************************************************************************
...
DESCRIPTION :
If the field is of type COMPUTED_FIELD_region_label, the
...
==============================================================================*/
{
	...
	ENTER(Computed_field_get_type_region_label);
	if (field && (field->type_string==computed_field_region_label_type_string)
		&& (data = (struct Computed_field_region_label_type_specific_data *)
		field->type_specific_data) && data->image)
	{
		*dimension = data->image->dimension;
		...
		{
			display_message(ERROR_MESSAGE,
				"Computed_field_get_type_region_label.  Unable to allocate vectors.");
			return_code = 0;
		}
	}
	else
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_get_type_region_label.  Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;

	return (return_code);
} /* Computed_field_get_type_region_label */

static int define_Computed_field_type_region_label(struct Parse_state *state,
	void *field_void, void *computed_field_region_label_package_void)
/*******************************************************************************
LAST MODIFIED : 4 December 2003

DESCRIPTION :
Converts <field> into type COMPUTED_FIELD_region_label (if it is not
...
==============================================================================*/
{
	...
	struct Computed_field_region_label_package
		*computed_field_region_label_package;
	struct Option_table *option_table;
	struct Set_Computed_field_conditional_data set_source_field_data,
		set_texture_coordinate_field_data;

	ENTER(define_Computed_field_type_region_label);
	if (state&&(field=(struct Computed_field *)field_void)&&
		(computed_field_region_label_package=
		(struct Computed_field_region_label_package *)
		computed_field_region_label_package_void))
	{
		return_code=1;
		...
		set_source_field_data.computed_field_manager =
			computed_field_region_label_package->computed_field_manager;
		...
		if (computed_field_region_label_type_string ==
			Computed_field_get_type_string(field))
		{
			return_code = Computed_field_get_type_region_label(field,
		...
		if (return_code)
		{
			...
			if (return_code)
			{
				return_code = Computed_field_set_type_region_label(field,
					source_field, texture_coordinate_field, dimension, sizes,
					computed_field_region_label_package->computed_field_manager,
					computed_field_region_label_package->root_region,
					computed_field_region_label_package->graphics_buffer_package);
			}
			if (!return_code)
			{
						...
						strcmp(PARSER_RECURSIVE_HELP_STRING,state->current_token)))
				{
					/* error */
					display_message(ERROR_MESSAGE,
						"define_Computed_field_type_region_label.  Failed");
				}
			}
			if (source_field)
			{
	...
	{
		display_message(ERROR_MESSAGE,
			"define_Computed_field_type_region_label.  Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;

	return (return_code);
} /* define_Computed_field_type_region_label */
...
{
	int return_code;
	static struct Computed_field_region_label_package
		computed_field_region_label_package;

	ENTER(Computed_field_register_types_region_label);
	if (computed_field_package)
	{
		computed_field_region_label_package.computed_field_manager =
			Computed_field_package_get_computed_field_manager(
				computed_field_package);
		computed_field_region_label_package.root_region = root_region;
		computed_field_region_label_package.graphics_buffer_package = graphics_buffer_package;
		return_code = Computed_field_package_add_type(computed_field_package,
			            computed_field_region_label_type_string,
			            define_Computed_field_type_region_label,
			            &computed_field_region_label_package);
	}
	else
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_register_types_region_label.  Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;
