...
==============================================================================*/
{
		...
		field->type_specific_data) )
	{
		...
		sprintf(temp_string1, " object_label %d", data->object_label);
		...
		if (return_code)
		{
			...
			if (return_code)
			{
				return_code = Computed_field_set_type_image_threshold(field,
					...
					computed_field_image_threshold_package->user_interface);
			}
			if (!return_code)
			{
...
==============================================================================*/
{
	...
	if (computed_field_package)
	{
		...
		computed_field_image_threshold_package.user_interface = user_interface;
