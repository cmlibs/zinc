/******************************************************************
  FILE: computed_field_gabor_filter.c

  LAST MODIFIED: 25 August 2004

  DESCRIPTION:Implement image filter with Gabor filter.
==================================================================*/
/* ***** BEGIN LICENSE BLOCK *****
...
static char computed_field_gabor_filter_type_string[] = "gabor_filter";

int Computed_field_is_type_gabor_filter(struct Computed_field *field)
/*******************************************************************************
...
==============================================================================*/
{
	...
	ENTER(Computed_field_is_type_gabor_filter);
	if (field)
	{
		return_code =
		  (field->type_string == computed_field_gabor_filter_type_string);
	}
	else
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_is_type_gabor_filter.  Missing field");
		return_code = 0;
	}

	return (return_code);
} /* Computed_field_is_type_gabor_filter */

static void Computed_field_gabor_filter_field_change(
...
==============================================================================*/
{
	...
	struct Computed_field_gabor_filter_type_specific_data *data;

	ENTER(Computed_field_gabor_filter_source_field_change);
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_gabor_filter_source_field_change.  "
			"Invalid arguments.");
	}
	LEAVE;
} /* Computed_field_gabor_filter_source_field_change */

static int Computed_field_gabor_filter_clear_type_specific(
	struct Computed_field *field)
/*******************************************************************************
LAST MODIFIED : 6 January 2004
...
==============================================================================*/
{
	...
	ENTER(Computed_field_gabor_filter_clear_type_specific);
	if (field && (data =
		(struct Computed_field_gabor_filter_type_specific_data *)
		field->type_specific_data))
	{
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_gabor_filter_clear_type_specific.  "
			"Invalid arguments.");
		return_code = 0;
	}
	LEAVE;
...
static void *Computed_field_gabor_filter_copy_type_specific(
...
==============================================================================*/
{
	struct Computed_field_gabor_filter_type_specific_data *destination,
		*source;

	ENTER(Computed_field_gabor_filter_copy_type_specific);
		...
		source_field->type_specific_data))
	{
			...
			ALLOCATE(destination->frequencies, FE_value, 2))
		{
			...
			destination->frequencies[0] = source->frequencies[0];
			...
			destination->computed_field_manager_callback_id =
				...
				Computed_field_gabor_filter_field_change, (void *)destination_field,
		...
		{
			display_message(ERROR_MESSAGE,
				"Computed_field_gabor_filter_copy_type_specific.  "
...
int Computed_field_gabor_filter_clear_cache_type_specific
   (struct Computed_field *field)
/*******************************************************************************
LAST MODIFIED : 6 January 2004
...
static int Computed_field_gabor_filter_type_specific_contents_match(
...
==============================================================================*/
{
	...
	ENTER(Computed_field_gabor_filter_type_specific_contents_match);
	...
	return (return_code);
} /* Computed_field_gabor_filter_type_specific_contents_match */

#define Computed_field_gabor_filter_is_defined_in_element \
...
#define Computed_field_gabor_filter_is_defined_at_node \
...
#define Computed_field_gabor_filter_has_numerical_components \
...
#define Computed_field_gabor_filter_not_in_use \
...
DESCRIPTION : Implement image filtering with Gabor filter.

==============================================================================*/
{
	char *storage;
	FE_value *data_index, *result_index, *max, *re_kernel, *im_kernel;
	int i, j, k, return_code, kernel_size, filter_size, storage_size;
	int radius, m;
	FE_value re_dot, im_dot, s, t, g;
	int *offsets, *cord;
	int image_step, kernel_step;
	FE_value theta;

	ENTER(Image_cache_gabor_filter);
	if (image && (image->depth > 0))
	{
		...
		if (ALLOCATE(re_kernel, FE_value, kernel_size) &&
		        ALLOCATE(im_kernel, FE_value, kernel_size) &&
			...
			ALLOCATE(max, FE_value, image->depth))
		{
			...
			for(j = 0; j < kernel_size; j++)
			{
				...
				{
				        cord[m] = ((int)((FE_value)j/((FE_value)kernel_step))) % filter_size;
					offsets[j] += (cord[m] - radius) * image_step * image->depth;
					kernel_step *= filter_size;
					image_step *= image->sizes[m];
				}
				s = ((FE_value)cord[0]) * cos(theta) + ((FE_value)cord[1]) * sin(theta);
				t = ((FE_value)cord[1]) * cos(theta) - ((FE_value)cord[0]) * sin(theta);
				g = 0.6 * pow(2.7, -0.5 * (s*s + 16.0 * t*t)/(sigma*sigma))/ (sigma*sigma);
				re_kernel[j] = g * cos(2.0 * M_PI *(((FE_value)cord[0]) * frequencies[0] + ((FE_value)cord[1]) * frequencies[1]));
				im_kernel[j] = g * sin(2.0 * M_PI *(((FE_value)cord[0]) * frequencies[0] + ((FE_value)cord[1]) * frequencies[1]));
			}
			...
			for (i = 0; i < storage_size / image->depth; i++)
			{
				for (k = 0; k < image->depth; k++)
				{
					...
					for (j = 0; j < kernel_size; j++)
					{
						...
						{
							re_dot += *(data_index + offsets[j] + k) * re_kernel[j];
							im_dot += *(data_index + offsets[j] + k) * im_kernel[j];
						}
					}
					result_index[k] = sqrt(re_dot * re_dot + im_dot * im_dot);
			...
			DEALLOCATE(re_kernel);
			DEALLOCATE(cord);
			DEALLOCATE(offsets);
			DEALLOCATE(im_kernel);
			DEALLOCATE(max);
		}
		else
		{
			display_message(ERROR_MESSAGE,
				"Image_cache_gabor_filter.  Not enough memory");
			return_code = 0;
		}
	}
	else
	{
		display_message(ERROR_MESSAGE, "Image_cache_gabor_filter.  "
			"Invalid arguments.");
		return_code=0;
	}
	LEAVE;
	...
	return (return_code);
} /* Image_cache_gabor_filter */

static int Computed_field_gabor_filter_evaluate_cache_at_node(
...
==============================================================================*/
{
	...
	ENTER(Computed_field_gabor_filter_evaluate_cache_at_node);
	if (field && node &&
		(data = (struct Computed_field_gabor_filter_type_specific_data *)field->type_specific_data))
	{
		return_code = 1;
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_gabor_filter_evaluate_cache_at_node.  "
			"Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;
...
static int Computed_field_gabor_filter_evaluate_cache_in_element(
...
==============================================================================*/
{
	...
	ENTER(Computed_field_gabor_filter_evaluate_cache_in_element);
		...
		data->image && (field->number_of_components == data->image->depth))
	{
		...
		if (!data->image->valid)
		{
			...
			return_code = Image_cache_gabor_filter(data->image, data->sigma,
			         data->direction, data->frequencies);
		}
		/* 3. Evaluate texture coordinates and copy image to field */
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_gabor_filter_evaluate_cache_in_element.  "
			"Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;
...
#define Computed_field_gabor_filter_evaluate_as_string_at_node \
...
#define Computed_field_gabor_filter_evaluate_as_string_in_element \
...
#define Computed_field_gabor_filter_set_values_at_node \
...
#define Computed_field_gabor_filter_set_values_in_element \
...
#define Computed_field_gabor_filter_get_native_discretization_in_element \
...
#define Computed_field_gabor_filter_find_element_xi \
...
{       
	...
	ENTER(Computed_field_gabor_filter_get_native_resolution);
	if (field && (data =
		(struct Computed_field_gabor_filter_type_specific_data *)
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_gabor_filter_get_native_resolution.  Missing field");
		return_code=0;
	}

	return (return_code);
} /* Computed_field_gabor_filter_get_native_resolution */


static int list_Computed_field_gabor_filter(
	struct Computed_field *field)
/*******************************************************************************
...
==============================================================================*/
{
	...
	ENTER(List_Computed_field_gabor_filter);
	if (field && (field->type_string==computed_field_gabor_filter_type_string)
		&& (data = (struct Computed_field_gabor_filter_type_specific_data *)
		field->type_specific_data))
	{
	...
	{
		display_message(ERROR_MESSAGE,
			"list_Computed_field_gabor_filter.  Invalid field");
		return_code = 0;
	}
	LEAVE;

	return (return_code);
} /* list_Computed_field_gabor_filter */

static char *Computed_field_gabor_filter_get_command_string(
	struct Computed_field *field)
/*******************************************************************************
LAST MODIFIED : 4 August 2004
...
==============================================================================*/
{
	...
	ENTER(Computed_field_gabor_filter_get_command_string);
		...
		field->type_specific_data) )
	{
		...
		append_string(&command_string,
			computed_field_gabor_filter_type_string, &error);
		...
		sprintf(temp_string, " direction %d %d",
		                    data->direction[0],data->direction[1]);
		append_string(&command_string, temp_string, &error);

		sprintf(temp_string, " frequencies %f %f",
		                    data->frequencies[0],data->frequencies[1]);
		append_string(&command_string, temp_string, &error);
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_gabor_filter_get_command_string.  Invalid field");
	}
	LEAVE;

	return (command_string);
} /* Computed_field_gabor_filter_get_command_string */

#define Computed_field_gabor_filter_has_multiple_times \
...
==============================================================================*/
{
	...
	ENTER(Computed_field_set_type_gabor_filter);
		...
		region && graphics_buffer_package)
	{
		...
		data = (struct Computed_field_gabor_filter_type_specific_data *)NULL;
			...
			Image_cache_update_data_storage(data->image))
		{
			...
			data->computed_field_manager_callback_id =
				MANAGER_REGISTER(Computed_field)(
				Computed_field_gabor_filter_field_change, (void *)field,
				computed_field_manager);

			field->type_specific_data = data;

			/* Set all the methods */
			COMPUTED_FIELD_ESTABLISH_METHODS(gabor_filter);
		}
		else
		{
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_set_type_gabor_filter.  Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;

	return (return_code);
} /* Computed_field_set_type_gabor_filter */
...
DESCRIPTION :
If the field is of type COMPUTED_FIELD_gabor_filter, the
...
==============================================================================*/
{
	...
	ENTER(Computed_field_get_type_gabor_filter);
	if (field && (field->type_string==computed_field_gabor_filter_type_string)
		&& (data = (struct Computed_field_gabor_filter_type_specific_data *)
		field->type_specific_data) && data->image)
	{
		*dimension = data->image->dimension;
			...
			&& ALLOCATE(*frequencies, FE_value, 2))
		{
			...
			*sigma = data->sigma;
			...
			(*frequencies)[0] = data->frequencies[0];
			(*frequencies)[1] = data->frequencies[1];
		...
		{
			display_message(ERROR_MESSAGE,
				"Computed_field_get_type_gabor_filter.  Unable to allocate vectors.");
			return_code = 0;
		}
	}
	else
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_get_type_gabor_filter.  Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;

	return (return_code);
} /* Computed_field_get_type_gabor_filter */

static int define_Computed_field_type_gabor_filter(struct Parse_state *state,
	void *field_void, void *computed_field_gabor_filter_package_void)
/*******************************************************************************
LAST MODIFIED : 4 August 2004

DESCRIPTION :
Converts <field> into type COMPUTED_FIELD_gabor_filter (if it is not
...
==============================================================================*/
{
	...
	if (state&&(field=(struct Computed_field *)field_void)&&
		(computed_field_gabor_filter_package=
		(struct Computed_field_gabor_filter_package *)
		computed_field_gabor_filter_package_void))
	{
		return_code=1;
		...
		if (return_code)
		{
			...
			if (!return_code)
			{
						...
						strcmp(PARSER_RECURSIVE_HELP_STRING,state->current_token)))
				{
					/* error */
					display_message(ERROR_MESSAGE,
						"define_Computed_field_type_gabor_filter.  Failed");
				}
			}
			if (source_field)
			{
	...
	{
		display_message(ERROR_MESSAGE,
			"define_Computed_field_type_gabor_filter.  Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;

	return (return_code);
} /* define_Computed_field_type_gabor_filter */
...
{
	int return_code;
	static struct Computed_field_gabor_filter_package
		computed_field_gabor_filter_package;

	ENTER(Computed_field_register_types_gabor_filter);
	if (computed_field_package)
	{
		computed_field_gabor_filter_package.computed_field_manager =
			Computed_field_package_get_computed_field_manager(
				computed_field_package);
		computed_field_gabor_filter_package.root_region = root_region;
		computed_field_gabor_filter_package.graphics_buffer_package = graphics_buffer_package;
		return_code = Computed_field_package_add_type(computed_field_package,
			            computed_field_gabor_filter_type_string,
			            define_Computed_field_type_gabor_filter,
			            &computed_field_gabor_filter_package);
	}
	else
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_register_types_gabor_filter.  Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;
