...
==============================================================================*/
{
	...
	FE_value SMI; /* structure model index */
        ...
        FE_value	maxrand;			/* value of maximum random num.	*/
	...
	if (image && (dimension == image->dimension) && (image->depth > 0))
	{
				...
				ALLOCATE(storage,char, out_storage_size * sizeof(FE_value)))
		{
			...
			{
        	        	rot1[irot] =  random() * M_PI / (FE_value)maxrand;
        	        	rot2[irot] =  random() * M_PI / (FE_value)maxrand;
        	        }
			...
			Xctr = (int)(((FE_value)xctr / ((FE_value)image->sizes[0] - 1.0)) * ((FE_value)output_sizes[0] - 1.0));
			Yctr = (int)(((FE_value)yctr / ((FE_value)image->sizes[1] - 1.0)) * ((FE_value)output_sizes[1] - 1.0));
			Zctr = (int)(((FE_value)zctr / ((FE_value)image->sizes[2] - 1.0)) * ((FE_value)output_sizes[2] - 1.0));
			for (i = 0; i < (out_storage_size / 12) ; i++)
			{
				if (i == ((Zctr * output_sizes[1] + Yctr) * output_sizes[0] + Xctr))
				{
					result_index[0] = x1;
					result_index[1] = y1;
					result_index[2] = z1;
