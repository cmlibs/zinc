...
int Computed_field_k_nearest_mean_get_native_resolution(struct Computed_field *field,
...
{       
	...
	ENTER(Computed_field_k_nearest_mean_get_native_resolution);
	if (field && (data =
		(struct Computed_field_k_nearest_mean_type_specific_data *)
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_k_nearest_mean_get_native_resolution.  Missing field");
		return_code=0;
	}

	return (return_code);
} /* Computed_field_k_nearest_mean_get_native_resolution */
...
int Computed_field_set_type_k_nearest_mean(struct Computed_field *field,
	...
	struct Computed_field *texture_coordinate_field,
	int radius, int k_number, int dimension, int *sizes,
	struct MANAGER(Computed_field) *computed_field_manager,
...
{
	char *current_token;
	int dimension, radius, k_number, return_code, *sizes;
		...
		if (return_code)
		{
			...
			if (return_code)
			{
				return_code = Computed_field_set_type_k_nearest_mean(field,
					source_field, texture_coordinate_field, radius, k_number, dimension, sizes,
					computed_field_k_nearest_mean_package->computed_field_manager,
