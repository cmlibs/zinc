/******************************************************************
  FILE: computed_field_haar_wavelet_decomp.c

  LAST MODIFIED: 15 July 2004

  DESCRIPTION:Implement image Haar wavelet decomposition
==================================================================*/
#include <math.h>
...
struct Computed_field_haar_wavelet_decomp_type_specific_data
{
...
static char computed_field_haar_wavelet_decomp_type_string[] = "haar_wavelet_decomp";

int Computed_field_is_type_haar_wavelet_decomp(struct Computed_field *field)
/*******************************************************************************
...
==============================================================================*/
{
	...
	ENTER(Computed_field_is_type_haar_wavelet_decomp);
	if (field)
	{
		return_code =
		  (field->type_string == computed_field_haar_wavelet_decomp_type_string);
	}
	else
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_is_type_haar_wavelet_decomp.  Missing field");
		return_code = 0;
	}

	return (return_code);
} /* Computed_field_is_type_haar_wavelet_decomp */

static void Computed_field_haar_wavelet_decomp_field_change(
...
==============================================================================*/
{
	...
	struct Computed_field_haar_wavelet_decomp_type_specific_data *data;

	ENTER(Computed_field_haar_wavelet_decomp_source_field_change);
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_haar_wavelet_decomp_source_field_change.  "
			"Invalid arguments.");
	}
	LEAVE;
} /* Computed_field_haar_wavelet_decomp_source_field_change */

static int Computed_field_haar_wavelet_decomp_clear_type_specific(
	struct Computed_field *field)
/*******************************************************************************
LAST MODIFIED : 10 December 2003
...
==============================================================================*/
{
	...
	ENTER(Computed_field_haar_wavelet_decomp_clear_type_specific);
	if (field && (data =
		(struct Computed_field_haar_wavelet_decomp_type_specific_data *)
		field->type_specific_data))
	{
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_haar_wavelet_decomp_clear_type_specific.  "
			"Invalid arguments.");
		return_code = 0;
	}
	LEAVE;
...
static void *Computed_field_haar_wavelet_decomp_copy_type_specific(
...
==============================================================================*/
{
	struct Computed_field_haar_wavelet_decomp_type_specific_data *destination,
		*source;

	ENTER(Computed_field_haar_wavelet_decomp_copy_type_specific);
	if (source_field && destination_field && (source =
		(struct Computed_field_haar_wavelet_decomp_type_specific_data *)
		source_field->type_specific_data))
	{
		if (ALLOCATE(destination,
			struct Computed_field_haar_wavelet_decomp_type_specific_data, 1))
		{
			...
			destination->computed_field_manager_callback_id =
				...
				Computed_field_haar_wavelet_decomp_field_change, (void *)destination_field,
		...
		{
			display_message(ERROR_MESSAGE,
				"Computed_field_haar_wavelet_decomp_copy_type_specific.  "
...
int Computed_field_haar_wavelet_decomp_clear_cache_type_specific
   (struct Computed_field *field)
/*******************************************************************************
LAST MODIFIED : 10 December 2003
...
static int Computed_field_haar_wavelet_decomp_type_specific_contents_match(
...
==============================================================================*/
{
	...
	ENTER(Computed_field_haar_wavelet_decomp_type_specific_contents_match);
	...
	return (return_code);
} /* Computed_field_haar_wavelet_decomp_type_specific_contents_match */

#define Computed_field_haar_wavelet_decomp_is_defined_in_element \
...
#define Computed_field_haar_wavelet_decomp_is_defined_at_node \
...
#define Computed_field_haar_wavelet_decomp_has_numerical_components \
...
#define Computed_field_haar_wavelet_decomp_not_in_use \
...
int Do_haar_1D_forward(FE_value *array, int w)
/**********************************************************************************
...
====================================================================================*/
{
	...
	if (ALLOCATE(avg, FE_value, w/2) && ALLOCATE(diff, FE_value, w/2))
	{
	        return_code = 1;

		for (i = 0; i < w /2; i++)
		{
		        avg[i] = (array[2*i] + array[2*i + 1]) / 2.0;
...
int Image_cache_haar_wavelet_decomp(struct Image_cache *image, int number_of_levels)
/*********************************************************************************
   LAST MODIFIED: 11 March 2004

   DESCRIPTION: Implement image wavelet decomposition with Haar wavelet.
==================================================================================*/
{
	...
	int return_code;
        FE_value M;
	int n, h, w;
	ENTER(Image_cache_haar_wavelet_decomp);
        if (image)
	{
	         return_code = 1;
		 /* allocate storage */
			...
			for (pass = 0; pass < image->depth; pass++)
			{
                                ...
                                for (j = 0; j < height; j++)
				{
					         ...
					         wt_index[k] = *(data_index + i * image->depth + pass);
                                        ...
                                        /* forward transform */
	                                        ...
	                                        /* for(n = width; n >= 4; n /= 2) */
						 Do_haar_1D_forward(wt_index, width);
                                ...
                                for (j = 0; j < width; j++)
				{
					...
					/* forward transform */
						 ...
						 Do_haar_1D_forward(wt_index, height);
	...
	LEAVE;
	return(return_code);;
}/* Image_cache_haar_wavelet_decomp */


static int Computed_field_haar_wavelet_decomp_evaluate_cache_at_node(
...
==============================================================================*/
{
	...
	ENTER(Computed_field_haar_wavelet_decomp_evaluate_cache_at_node);
	if (field && node &&
		(data = (struct Computed_field_haar_wavelet_decomp_type_specific_data *)field->type_specific_data))
	{
		return_code = 1;
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_haar_wavelet_decomp_evaluate_cache_at_node.  "
			"Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;
...
static int Computed_field_haar_wavelet_decomp_evaluate_cache_in_element(
...
==============================================================================*/
{
	...
	ENTER(Computed_field_haar_wavelet_decomp_evaluate_cache_in_element);
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_haar_wavelet_decomp_evaluate_cache_in_element.  "
			"Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;
...
#define Computed_field_haar_wavelet_decomp_evaluate_as_string_at_node \
...
#define Computed_field_haar_wavelet_decomp_evaluate_as_string_in_element \
...
#define Computed_field_haar_wavelet_decomp_set_values_at_node \
...
#define Computed_field_haar_wavelet_decomp_set_values_in_element \
...
#define Computed_field_haar_wavelet_decomp_get_native_discretization_in_element \
...
#define Computed_field_haar_wavelet_decomp_find_element_xi \
...
{       
	...
	ENTER(Computed_field_haar_wavelet_decomp_get_native_resolution);
	if (field && (data =
		(struct Computed_field_haar_wavelet_decomp_type_specific_data *)
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_haar_wavelet_decomp_get_native_resolution.  Missing field");
		return_code=0;
	}

	return (return_code);
} /* Computed_field_haar_wavelet_decomp_get_native_resolution */

static int list_Computed_field_haar_wavelet_decomp(
	struct Computed_field *field)
/*******************************************************************************
...
==============================================================================*/
{
	...
	ENTER(List_Computed_field_haar_wavelet_decomp);
	if (field && (field->type_string==computed_field_haar_wavelet_decomp_type_string)
		&& (data = (struct Computed_field_haar_wavelet_decomp_type_specific_data *)
		field->type_specific_data))
	{
	...
	{
		display_message(ERROR_MESSAGE,
			"list_Computed_field_haar_wavelet_decomp.  Invalid field");
		return_code = 0;
	}
	LEAVE;

	return (return_code);
} /* list_Computed_field_haar_wavelet_decomp */

static char *Computed_field_haar_wavelet_decomp_get_command_string(
	struct Computed_field *field)
/*******************************************************************************
LAST MODIFIED : 4 December 2003
...
==============================================================================*/
{
	...
	ENTER(Computed_field_haar_wavelet_decomp_get_command_string);
		...
		field->type_specific_data) )
	{
		...
		append_string(&command_string,
			computed_field_haar_wavelet_decomp_type_string, &error);
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_haar_wavelet_decomp_get_command_string.  Invalid field");
	}
	LEAVE;

	return (command_string);
} /* Computed_field_haar_wavelet_decomp_get_command_string */

#define Computed_field_haar_wavelet_decomp_has_multiple_times \
...
DESCRIPTION :
Converts <field> to type COMPUTED_FIELD_haar_wavelet_decomp with the supplied
fields, <source_field> and <texture_coordinate_field>.  The <number_of_levels> specifies
the number of decomposition layers.  The <dimension> is the
...
==============================================================================*/
{
	...
	ENTER(Computed_field_set_type_haar_wavelet_decomp);
		...
		region && graphics_buffer_package)
	{
		...
		data = (struct Computed_field_haar_wavelet_decomp_type_specific_data *)NULL;
			...
			Image_cache_update_data_storage(data->image))
		{
			...
			data->computed_field_manager_callback_id =
				MANAGER_REGISTER(Computed_field)(
				Computed_field_haar_wavelet_decomp_field_change, (void *)field,
				computed_field_manager);

			field->type_specific_data = data;

			/* Set all the methods */
			COMPUTED_FIELD_ESTABLISH_METHODS(haar_wavelet_decomp);
		}
		else
		{
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_set_type_haar_wavelet_decomp.  Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;

	return (return_code);
} /* Computed_field_set_type_haar_wavelet_decomp */
...
DESCRIPTION :
If the field is of type COMPUTED_FIELD_haar_wavelet_decomp, the
...
==============================================================================*/
{
	...
	ENTER(Computed_field_get_type_haar_wavelet_decomp);
	if (field && (field->type_string==computed_field_haar_wavelet_decomp_type_string)
		&& (data = (struct Computed_field_haar_wavelet_decomp_type_specific_data *)
		field->type_specific_data) && data->image)
	{
		*dimension = data->image->dimension;
		...
		{
			display_message(ERROR_MESSAGE,
				"Computed_field_get_type_haar_wavelet_decomp.  Unable to allocate vectors.");
			return_code = 0;
		}
	}
	else
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_get_type_haar_wavelet_decomp.  Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;

	return (return_code);
} /* Computed_field_get_type_haar_wavelet_decomp */

static int define_Computed_field_type_haar_wavelet_decomp(struct Parse_state *state,
	void *field_void, void *computed_field_haar_wavelet_decomp_package_void)
/*******************************************************************************
LAST MODIFIED : 4 December 2003

DESCRIPTION :
Converts <field> into type COMPUTED_FIELD_haar_wavelet_decomp (if it is not
...
==============================================================================*/
{
	...
	ENTER(define_Computed_field_type_haar_wavelet_decomp);
	if (state&&(field=(struct Computed_field *)field_void)&&
		(computed_field_haar_wavelet_decomp_package=
		(struct Computed_field_haar_wavelet_decomp_package *)
		computed_field_haar_wavelet_decomp_package_void))
	{
		return_code=1;
		...
		if (computed_field_haar_wavelet_decomp_type_string ==
		...
		if (return_code)
		{
			...
			if (!return_code)
			{
						...
						strcmp(PARSER_RECURSIVE_HELP_STRING,state->current_token)))
				{
					/* error */
					display_message(ERROR_MESSAGE,
						"define_Computed_field_type_haar_wavelet_decomp.  Failed");
				}
			}
			if (source_field)
			{
	...
	{
		display_message(ERROR_MESSAGE,
			"define_Computed_field_type_haar_wavelet_decomp.  Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;

	return (return_code);
} /* define_Computed_field_type_haar_wavelet_decomp */
...
{
	int return_code;
	static struct Computed_field_haar_wavelet_decomp_package
		computed_field_haar_wavelet_decomp_package;

	ENTER(Computed_field_register_types_haar_wavelet_decomp);
	if (computed_field_package)
	{
		computed_field_haar_wavelet_decomp_package.computed_field_manager =
			Computed_field_package_get_computed_field_manager(
				computed_field_package);
		computed_field_haar_wavelet_decomp_package.root_region = root_region;
		computed_field_haar_wavelet_decomp_package.graphics_buffer_package = graphics_buffer_package;
		return_code = Computed_field_package_add_type(computed_field_package,
			            computed_field_haar_wavelet_decomp_type_string,
			            define_Computed_field_type_haar_wavelet_decomp,
			            &computed_field_haar_wavelet_decomp_package);
	}
	else
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_register_types_haar_wavelet_decomp.  Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;
