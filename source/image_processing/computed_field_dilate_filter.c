...
struct Computed_field_dilate_filter_package
/*******************************************************************************
...
struct Computed_field_dilate_filter_type_specific_data
{
	/* The size of the dilate filter window */
	int radius;
...
static char computed_field_dilate_filter_type_string[] = "dilate_filter";

int Computed_field_is_type_dilate_filter(struct Computed_field *field)
/*******************************************************************************
...
==============================================================================*/
{
	...
	ENTER(Computed_field_is_type_dilate_filter);
	if (field)
	{
		return_code =
		  (field->type_string == computed_field_dilate_filter_type_string);
	}
	else
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_is_type_dilate_filter.  Missing field");
		return_code = 0;
	}

	return (return_code);
} /* Computed_field_is_type_dilate_filter */

static void Computed_field_dilate_filter_field_change(
...
==============================================================================*/
{
	...
	ENTER(Computed_field_dilate_filter_source_field_change);
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_dilate_filter_source_field_change.  "
			"Invalid arguments.");
	}
	LEAVE;
} /* Computed_field_dilate_filter_source_field_change */

static int Computed_field_dilate_filter_clear_type_specific(
	struct Computed_field *field)
/*******************************************************************************
LAST MODIFIED : 6 January 2004
...
==============================================================================*/
{
	...
	ENTER(Computed_field_dilate_filter_clear_type_specific);
	if (field && (data =
		(struct Computed_field_dilate_filter_type_specific_data *)
		field->type_specific_data))
	{
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_dilate_filter_clear_type_specific.  "
			"Invalid arguments.");
		return_code = 0;
	}
	LEAVE;
...
static void *Computed_field_dilate_filter_copy_type_specific(
...
==============================================================================*/
{
	struct Computed_field_dilate_filter_type_specific_data *destination,
		*source;

	ENTER(Computed_field_dilate_filter_copy_type_specific);
	if (source_field && destination_field && (source =
		(struct Computed_field_dilate_filter_type_specific_data *)
		source_field->type_specific_data))
	{
		if (ALLOCATE(destination,
			struct Computed_field_dilate_filter_type_specific_data, 1))
		{
			...
			destination->computed_field_manager_callback_id =
				...
				Computed_field_dilate_filter_field_change, (void *)destination_field,
		...
		{
			display_message(ERROR_MESSAGE,
				"Computed_field_dilate_filter_copy_type_specific.  "
...
int Computed_field_dilate_filter_clear_cache_type_specific
   (struct Computed_field *field)
/*******************************************************************************
LAST MODIFIED : 6 January 2004
...
static int Computed_field_dilate_filter_type_specific_contents_match(
...
==============================================================================*/
{
	...
	ENTER(Computed_field_dilate_filter_type_specific_contents_match);
	...
	return (return_code);
} /* Computed_field_dilate_filter_type_specific_contents_match */

#define Computed_field_dilate_filter_is_defined_in_element \
...
#define Computed_field_dilate_filter_is_defined_at_node \
...
#define Computed_field_dilate_filter_has_numerical_components \
...
#define Computed_field_dilate_filter_not_in_use \
...
static int Image_cache_dilate_filter(struct Image_cache *image, int radius)
/*******************************************************************************
...
==============================================================================*/
{
	...
	if (image && (image->dimension > 0) && (image->depth > 0))
	{
		...
		if (ALLOCATE(Di, FE_value, image->depth) &&
			...
			for (i = 0 ; return_code && i < storage_size / image->depth ; i++)
			{
				for (k = 0 ; k < image->depth ; k++)
				{
                                        ...
                                        for(j = 0 ; j < kernel_size ; j++)
					{
						...
						{
						          /*wrapping around */
						         Di[k] = my_Max(Di[k], *(data_index + offsets[j] + storage_size +k));
						}
						...
						{
							/*standard */
						        Di[k] = my_Max(Di[k], *(data_index + offsets[j] + k));
						}
					}
					result_index[k] = Di[k];
				}
		...
		{
			display_message(ERROR_MESSAGE,
				"Image_cache_dilate_filter.  Not enough memory");
			return_code = 0;
		}
	}
	else
	{
		display_message(ERROR_MESSAGE, "Image_cache_dilate_filter.  "
			"Invalid arguments.");
		return_code=0;
	}
	LEAVE;
	...
	return (return_code);
} /* Image_cache_dilate_filter */

static int Computed_field_dilate_filter_evaluate_cache_at_node(
...
==============================================================================*/
{
	...
	ENTER(Computed_field_dilate_filter_evaluate_cache_at_node);
	if (field && node &&
		(data = (struct Computed_field_dilate_filter_type_specific_data *)field->type_specific_data))
	{
		return_code = 1;
		...
		if (!data->image->valid)
		{
			...
			return_code = Image_cache_dilate_filter(data->image, data->radius);
		}
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_dilate_filter_evaluate_cache_at_node.  "
			"Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;
...
static int Computed_field_dilate_filter_evaluate_cache_in_element(
...
==============================================================================*/
{
	...
	ENTER(Computed_field_dilate_filter_evaluate_cache_in_element);
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_dilate_filter_evaluate_cache_in_element.  "
			"Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;
...
#define Computed_field_dilate_filter_evaluate_as_string_at_node \
...
#define Computed_field_dilate_filter_evaluate_as_string_in_element \
...
#define Computed_field_dilate_filter_set_values_at_node \
...
#define Computed_field_dilate_filter_set_values_in_element \
...
#define Computed_field_dilate_filter_get_native_discretization_in_element \
...
#define Computed_field_dilate_filter_find_element_xi \
...
int Computed_field_dilate_filter_get_native_resolution(struct Computed_field *field,
...
{       
	...
	ENTER(Computed_field_dilate_filter_get_native_resolution);
	if (field && (data =
		(struct Computed_field_dilate_filter_type_specific_data *)
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_dilate_filter_get_native_resolution.  Missing field");
		return_code=0;
	}

	return (return_code);
} /* Computed_field_dilate_filter_get_native_resolution */

static int list_Computed_field_dilate_filter(
	struct Computed_field *field)
/*******************************************************************************
...
==============================================================================*/
{
	...
	ENTER(List_Computed_field_dilate_filter);
	if (field && (field->type_string==computed_field_dilate_filter_type_string)
		&& (data = (struct Computed_field_dilate_filter_type_specific_data *)
		field->type_specific_data))
	{
	...
	{
		display_message(ERROR_MESSAGE,
			"list_Computed_field_dilate_filter.  Invalid field");
		return_code = 0;
	}
	LEAVE;

	return (return_code);
} /* list_Computed_field_dilate_filter */

static char *Computed_field_dilate_filter_get_command_string(
	struct Computed_field *field)
/*******************************************************************************
LAST MODIFIED : 4 December 2003
...
==============================================================================*/
{
	...
	ENTER(Computed_field_dilate_filter_get_command_string);
		...
		field->type_specific_data) )
	{
		...
		append_string(&command_string,
			computed_field_dilate_filter_type_string, &error);
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_dilate_filter_get_command_string.  Invalid field");
	}
	LEAVE;

	return (command_string);
} /* Computed_field_dilate_filter_get_command_string */

#define Computed_field_dilate_filter_has_multiple_times \
...
int Computed_field_set_type_dilate_filter(struct Computed_field *field,
...
DESCRIPTION :
Converts <field> to type COMPUTED_FIELD_dilate_filter with the supplied
...
==============================================================================*/
{
	...
	ENTER(Computed_field_set_type_dilate_filter);
		...
		region && graphics_buffer_package)
	{
		...
		data = (struct Computed_field_dilate_filter_type_specific_data *)NULL;
		if (ALLOCATE(source_fields, struct Computed_field *, number_of_source_fields) &&
			ALLOCATE(data, struct Computed_field_dilate_filter_type_specific_data, 1) &&
			...
			Image_cache_update_data_storage(data->image))
		{
			...
			data->computed_field_manager_callback_id =
				MANAGER_REGISTER(Computed_field)(
				Computed_field_dilate_filter_field_change, (void *)field,
				computed_field_manager);

			field->type_specific_data = data;

			/* Set all the methods */
			COMPUTED_FIELD_ESTABLISH_METHODS(dilate_filter);
		}
		else
		{
	...
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_set_type_dilate_filter.  Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;

	return (return_code);
} /* Computed_field_set_type_dilate_filter */

int Computed_field_get_type_dilate_filter(struct Computed_field *field,
	struct Computed_field **source_field,
	struct Computed_field **texture_coordinate_field,
...
DESCRIPTION :
If the field is of type COMPUTED_FIELD_dilate_filter, the
...
==============================================================================*/
{
	...
	ENTER(Computed_field_get_type_dilate_filter);
	if (field && (field->type_string==computed_field_dilate_filter_type_string)
		&& (data = (struct Computed_field_dilate_filter_type_specific_data *)
		field->type_specific_data) && data->image)
	{
		*dimension = data->image->dimension;
		...
		{
			display_message(ERROR_MESSAGE,
				"Computed_field_get_type_dilate_filter.  Unable to allocate vectors.");
			return_code = 0;
		}
	}
	else
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_get_type_dilate_filter.  Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;

	return (return_code);
} /* Computed_field_get_type_dilate_filter */

static int define_Computed_field_type_dilate_filter(struct Parse_state *state,
	void *field_void, void *computed_field_dilate_filter_package_void)
/*******************************************************************************
LAST MODIFIED : 4 December 2003

DESCRIPTION :
Converts <field> into type COMPUTED_FIELD_dilate_filter (if it is not
...
==============================================================================*/
{
	...
	struct Computed_field_dilate_filter_package
		*computed_field_dilate_filter_package;
	struct Option_table *option_table;
	struct Set_Computed_field_conditional_data set_source_field_data,
		set_texture_coordinate_field_data;

	ENTER(define_Computed_field_type_dilate_filter);
	if (state&&(field=(struct Computed_field *)field_void)&&
		(computed_field_dilate_filter_package=
		(struct Computed_field_dilate_filter_package *)
		computed_field_dilate_filter_package_void))
	{
		return_code=1;
		...
		set_source_field_data.computed_field_manager =
			computed_field_dilate_filter_package->computed_field_manager;
		...
		if (computed_field_dilate_filter_type_string ==
			Computed_field_get_type_string(field))
		{
			return_code = Computed_field_get_type_dilate_filter(field,
		...
		if (return_code)
		{
			...
			if (return_code)
			{
				return_code = Computed_field_set_type_dilate_filter(field,
					source_field, texture_coordinate_field, radius, dimension,
					sizes, minimums, maximums, element_dimension,
					computed_field_dilate_filter_package->computed_field_manager,
					computed_field_dilate_filter_package->root_region,
					computed_field_dilate_filter_package->graphics_buffer_package);
			}
			if (!return_code)
			{
						...
						strcmp(PARSER_RECURSIVE_HELP_STRING,state->current_token)))
				{
					/* error */
					display_message(ERROR_MESSAGE,
						"define_Computed_field_type_dilate_filter.  Failed");
				}
			}
			if (source_field)
			{
	...
	{
		display_message(ERROR_MESSAGE,
			"define_Computed_field_type_dilate_filter.  Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;

	return (return_code);
} /* define_Computed_field_type_dilate_filter */

int Computed_field_register_types_dilate_filter(
...
{
	int return_code;
	static struct Computed_field_dilate_filter_package
		computed_field_dilate_filter_package;

	ENTER(Computed_field_register_types_dilate_filter);
	if (computed_field_package)
	{
		computed_field_dilate_filter_package.computed_field_manager =
			Computed_field_package_get_computed_field_manager(
				computed_field_package);
		computed_field_dilate_filter_package.root_region = root_region;
		computed_field_dilate_filter_package.graphics_buffer_package = graphics_buffer_package;
		return_code = Computed_field_package_add_type(computed_field_package,
			            computed_field_dilate_filter_type_string,
			            define_Computed_field_type_dilate_filter,
			            &computed_field_dilate_filter_package);
	}
	else
	{
		display_message(ERROR_MESSAGE,
			"Computed_field_register_types_dilate_filter.  Invalid argument(s)");
		return_code = 0;
	}
	LEAVE;
