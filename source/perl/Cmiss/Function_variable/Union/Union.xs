...
#include <string.h>
#include "api/zn_function_variable_union.h"
#include "typemap.h"

MODULE = Cmiss::Function_variable::Union  PACKAGE = Cmiss::Function_variable::Union  PREFIX = Cmiss_function_variable_union_
	...
	CODE:
			...
			to zero) */
		{
			...
			if (variables_array&&(0<(number_of_variables=av_len(variables_array)+1)))
			{
				if (variables=Cmiss_function_variable_list_create())
				{
					...
					while ((i<number_of_variables)&&variables)
					{
						...
						else
						{
							...
							Perl_croak(aTHX_ "Cmiss::Function_variable::Union::new.  "
								"Not a variable");
						}
					}
				}
				if (variables)
				{
					RETVAL=Cmiss_function_variable_union_create(variables);
				}
				Cmiss_function_variable_list_destroy(&variables);
			}
		}
