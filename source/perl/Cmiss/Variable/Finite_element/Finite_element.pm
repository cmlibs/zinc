package Cmiss::Variable::Finite_element;
...
# This allows declaration	use Cmiss::Variable::Finite_element ':all';
...
sub AUTOLOAD {
    ...
    croak "&Cmiss::Variable::Finite_element::constant not defined" if $constname eq 'constant';
...
sub new
{
	...
	if (defined($name)&&($name))
	{
		...
		if (defined($region)&&($region))
		{
			...
			if ($component_name)
			{
				$objref=create($region,$path,$name,$component_name);
			}
			else
			{
				$objref=create($region,$path,$name);
			}
			if ($objref)
			{
				bless $objref,$class;
...
require XSLoader;
XSLoader::load('Cmiss::Variable::Finite_element', $VERSION);
...
Cmiss::Variable::Finite_element - Perl extension for Cmiss finite element variables
...
=head1 ABSTRACT
  ...
  This should be the abstract for Cmiss::Variable::Finite_element.
...
Stub documentation for Cmiss::Variable::Finite_element, created by h2xs. It looks like
