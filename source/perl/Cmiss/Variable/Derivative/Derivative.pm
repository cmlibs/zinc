package Cmiss::Variable::Derivative;
...
# This allows declaration	use Cmiss::Variable::Derivative ':all';
...
sub AUTOLOAD {
    ...
    croak "&Cmiss::Variable::Derivative::constant not defined" if $constname eq 'constant';
...
{
	my ($class, %arg) = @_;
	my ($dependent,$independent,$name,$objref);
	...
	if ($name)
	{
		...
		if ($dependent)
		{
			...
			if ($independent)
			{
				$objref=create($name,$dependent,$independent);
				if ($objref)
				{
					bless $objref,$class;
				}
...
require XSLoader;
XSLoader::load('Cmiss::Variable::Derivative', $VERSION);
...
Cmiss::Variable::Derivative - Perl extension for Cmiss derivative variables
...
=head1 ABSTRACT
  ...
  This should be the abstract for Cmiss::Variable::Derivative.
...
Stub documentation for Cmiss::Variable::Derivative, created by h2xs. It looks like
