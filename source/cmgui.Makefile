...
API_DIRECTORY_INC = -I$(SOURCE_PATH)/api
...
FIELDML_LIB = $(CMISS_ROOT)/third_party/$(LIB_ARCH_DIR)/lib/$(FIELDML_LIB_PREFIX)fieldml_api$(FIELDML_LIB_SUFFIX) \
	 $(CMISS_ROOT)/third_party/$(LIB_ARCH_DIR)/lib/$(FIELDML_LIB_PREFIX)fieldml_io_api$(FIELDML_LIB_SUFFIX)
...
ifneq ($(USE_OPENCASCADE),true)
   ...
   OPENCASCADE_DEFINES += -DUSE_OPENCASCADE -DHAVE_CONFIG_H
   OPENCASCADE_PATH = $(CMISS_ROOT)/occ
   OPENCASCADE_INC = -I$(OPENCASCADE_PATH) -I$(OPENCASCADE_PATH)/inc
   OPENCASCADE_LIB = -L$(OPENCASCADE_PATH)/$(SYSNAME)/lib -lTKV3d -lTKIGES -lTKSTEP -lTKXDEIGES -lTKXDESTEP
   ifeq ($(LD_RUN_PATH),)
      export LD_RUN_PATH = $(OPENCASCADE_PATH)/$(SYSNAME)/lib
   else
      LD_RUN_PATH = $(OPENCASCADE_PATH)/$(SYSNAME)/lib:$(LD_RUN_PATH)
   endif
   ifeq ($(INSTRUCTION),x86_64)
      OPENCASCADE_DEFINES += -D_OCC64
      COMPILE_FLAGS += -m64
   endif
endif # !OPENCASCADE
...
ALL_INCLUDES = $(SOURCE_DIRECTORY_INC) $(API_DIRECTORY_INC) \
...
COMPUTED_FIELD_CORE_SRCS =\
	...
	computed_field/field_module.cpp \
...
OCC_SRCS += cad/occpartfactory.cpp \
	...
	cad/curve.cpp \
	cad/surface.cpp \
	cad/transformoccmodel.cpp \
	cad/geometricshape.cpp \
	cad/topologicalshape.cpp \
	cad/topologicalshaperoot.cpp \
	cad/fileimportexport.cpp \
	cad/cad_geometry_to_graphics_object.cpp \
	cad/computed_field_cad_geometry.cpp \
	cad/computed_field_cad_topology.cpp
else
OCC_SRCS =
endif
