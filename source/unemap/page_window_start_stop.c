	...
	switch (message_identifier)
	{
		...
		case WM_CREATE:
		{
			...
			i16 channel_vector[NUMBER_OF_CHANNELS_ON_NI_CARD],
				gain_vector[NUMBER_OF_CHANNELS_ON_NI_CARD],i,time_base;
	...
	if (debug_nidaq=fopen("nidaq.deb","w"))
	{
		...
		fclose(debug_nidaq);
			...
			if (0==status)
			{
				...
				/* search for NI cards */
				number_of_ni_cards=2;
					/*???DB.  To be done */
				/* allocate memory for cards */
				if (ALLOCATE(ni_cards,struct NI_card,number_of_ni_cards))
				{
					/* assign the device numbers */
						/*???DB.  To be done */
					(ni_cards[0]).device_number=1;
					(ni_cards[0]).memory_object=(HGLOBAL)NULL;
					(ni_cards[0]).hardware_buffer=(i16 *)NULL;
					(ni_cards[0]).time_base=time_base;
					(ni_cards[0]).sampling_interval=sampling_interval;
					(ni_cards[0]).hardware_buffer_size=hardware_buffer_size;
					(ni_cards[1]).device_number=2;
					(ni_cards[1]).memory_object=(HGLOBAL)NULL;
					(ni_cards[1]).hardware_buffer=(i16 *)NULL;
					(ni_cards[1]).time_base=time_base;
					(ni_cards[1]).sampling_interval=sampling_interval;
					(ni_cards[1]).hardware_buffer_size=hardware_buffer_size;
					/* configure cards */
					i=0;
/*???debug */
{
...
/*???debug */
{
	...
	if (debug_nidaq=fopen("nidaq.deb","a"))
	{
		...
		fclose(debug_nidaq);
									...
									status=DAQ_StopTrigger_Config(
										(ni_cards[i]).device_number,(i16)1,
										(u32)(NUMBER_OF_CHANNELS_ON_NI_CARD*2));
/*???debug */
{
	FILE *debug_nidaq;
	...
	if (debug_nidaq=fopen("nidaq.deb","a"))
	{
		fprintf(debug_nidaq,"DAQ_StopTrigger_Config=%d\n",status);
		fclose(debug_nidaq);
	}
}
									status=Select_Signal((ni_cards[i]).device_number,
										ND_IN_STOP_TRIGGER,ND_RTSI_1,ND_LOW_TO_HIGH);
/*???debug */
{
	FILE *debug_nidaq;
	...
	if (debug_nidaq=fopen("nidaq.deb","a"))
	{
		fprintf(debug_nidaq,"Select_Signal=%d\n",status);
		fclose(debug_nidaq);
	}
}
									if (0==status)
...
/*???debug */
{
	...
	{
		fprintf(debug_nidaq,
			"Select_Signal(0,ND_RTSI_CLOCK,ND_BOARD_CLOCK,ND_DONT_CARE)=%d\n",status);
		fclose(debug_nidaq);
	}
}
														if (0==status)
														{
															/* set the first card to output the start
																trigger */
/*															status=Select_Signal((ni_cards[i]).device_number,
																ND_RTSI_0,ND_IN_START_TRIGGER,ND_LOW_TO_HIGH);*/
															status=Select_Signal((ni_cards[i]).device_number,
																ND_IN_START_TRIGGER,ND_RTSI_0,ND_LOW_TO_HIGH);
/*???debug */
{
	FILE *debug_nidaq;
	...
	{
		fprintf(debug_nidaq,
			"Select_Signal(0,ND_RTSI_0,ND_IN_START_TRIGGER,ND_LOW_TO_HIGH)=%d\n",
			status);
		fclose(debug_nidaq);
	}
}
/*															status=DAQ_StopTrigger_Config(
																(ni_cards[i]).device_number,(i16)1,
																(u32)NUMBER_OF_CHANNELS_ON_NI_CARD);*/
/*???debug */
{
	FILE *debug_nidaq;
...
/*															status=Select_Signal((ni_cards[i]).device_number,
																ND_IN_STOP_TRIGGER,ND_RTSI_1,ND_LOW_TO_HIGH);*/
/*															status=DAQ_StopTrigger_Config(
																(ni_cards[i]).device_number,(i16)1,(u32)0);
															status=Select_Signal((ni_cards[i]).device_number,
																ND_RTSI_1,ND_IN_STOP_TRIGGER,ND_LOW_TO_HIGH);*/
/*???debug */
{
	FILE *debug_nidaq;
	...
	{
		fprintf(debug_nidaq,
			"Select_Signal(0,ND_IN_STOP_TRIGGER,ND_RTSI_1,ND_LOW_TO_HIGH)=%d\n",
			status);
		fclose(debug_nidaq);
	}
}
															...
															{
																display_message(ERROR_MESSAGE,
"Page_window_drawing_area_class_proc.  Select_Signal(0,ND_RTSI_0,ND_OUT_START_TRIGGER,ND_LOW_TO_HIGH) failed");
															}
														}
														else
														{
															display_message(ERROR_MESSAGE,
"Page_window_drawing_area_class_proc.  Select_Signal(0,ND_RTSI_CLOCK,ND_BOARD_CLOCK,ND_DONT_CARE) failed");
														}
													}
												}
												else
...
/*???debug */
{
	...
	{
		fprintf(debug_nidaq,
			"Select_Signal(>0,ND_BOARD_CLOCK,ND_RTSI_CLOCK,ND_DONT_CARE)=%d\n",
			status);
		fclose(debug_nidaq);
	}
}
													if (0==status)
													{
														/* set the other cards to input the start trigger */
														status=Select_Signal((ni_cards[i]).device_number,
															ND_IN_START_TRIGGER,ND_RTSI_0,ND_LOW_TO_HIGH);
/*???debug */
{
	...
	{
		fprintf(debug_nidaq,
			"Select_Signal(>0,ND_IN_START_TRIGGER,ND_RTSI_0,ND_LOW_TO_HIGH)=%d\n",
			status);
		fclose(debug_nidaq);
	}
}
	...
	if (debug_nidaq=fopen("nidaq.deb","a"))
	{
		...
		fclose(debug_nidaq);
								...
								"Page_window_drawing_area_class_proc.  SCAN_Start(>0) failed");
														...
														{
															display_message(ERROR_MESSAGE,
"Page_window_drawing_area_class_proc.  Select_Signal(>0,ND_IN_START_TRIGGER,ND_RTSI_0,ND_LOW_TO_HIGH) failed");
														}
													}
													else
													{
														display_message(ERROR_MESSAGE,
"Page_window_drawing_area_class_proc.  Select_Signal(>0,ND_BOARD_CLOCK,ND_RTSI_CLOCK,ND_DONT_CARE) failed");
													}
												}
											}
											if (0==status)
...
/*		SCAN_Sequence_Retrieve((ni_cards[0]).device_number,
		...
		fclose(debug_nidaq);
				...
				status=GPCTR_Control((ni_cards[0]).device_number,ND_COUNTER_0,ND_RESET);
/*???debug */
{
	FILE *debug_nidaq;
	...
	if (debug_nidaq=fopen("nidaq.deb","a"))
	{
		fprintf(debug_nidaq,"GPCTR_Control=%d\n",status);
		fclose(debug_nidaq);
	}
}
				status=GPCTR_Set_Application((ni_cards[0]).device_number,ND_COUNTER_0,
					ND_SINGLE_PULSE_GNR);
/*???debug */
{
	FILE *debug_nidaq;
	...
	if (debug_nidaq=fopen("nidaq.deb","a"))
	{
		fprintf(debug_nidaq,"GPCTR_Set_Application=%d\n",status);
		fclose(debug_nidaq);
	}
}
				status=GPCTR_Change_Parameter((ni_cards[0]).device_number,ND_COUNTER_0,
					ND_COUNT_1,3);
/*???debug */
{
	FILE *debug_nidaq;
	...
	if (debug_nidaq=fopen("nidaq.deb","a"))
	{
		fprintf(debug_nidaq,"GPCTR_Change_Parameter=%d\n",status);
		fclose(debug_nidaq);
	}
}
				status=GPCTR_Change_Parameter((ni_cards[0]).device_number,ND_COUNTER_0,
...
/*???debug */
{
	...
	if (debug_nidaq=fopen("nidaq.deb","a"))
	{
		...
		fclose(debug_nidaq);
				...
				status=Select_Signal((ni_cards[0]).device_number,ND_RTSI_0,
					ND_GPCTR0_SOURCE,ND_LOW_TO_HIGH);
/*				status=Select_Signal((ni_cards[0]).device_number,ND_PFI_1,
					ND_GPCTR0_SOURCE,ND_LOW_TO_HIGH);*/
/*???debug */
{
	FILE *debug_nidaq;
	...
	if (debug_nidaq=fopen("nidaq.deb","a"))
	{
		...
		fclose(debug_nidaq);
				...
				status=GPCTR_Control((ni_cards[0]).device_number,ND_COUNTER_0,
					ND_PROGRAM);
/*???debug */
{
	FILE *debug_nidaq;
...
/*???debug */
{
	...
	if (debug_nidaq=fopen("nidaq.deb","a"))
	{
		...
		fclose(debug_nidaq);
				...
				status=Select_Signal((ni_cards[0]).device_number,ND_RTSI_1,
					ND_GPCTR0_SOURCE,ND_LOW_TO_HIGH);
/*				status=Select_Signal((ni_cards[0]).device_number,ND_RTSI_0,
					ND_GPCTR0_SOURCE,ND_LOW_TO_HIGH);*/
/*???debug */
...
/*???debug */
{
	...
	if (debug_nidaq=fopen("nidaq.deb","a"))
	{
		...
		fclose(debug_nidaq);
				...
				/* wait until signal has been sent */
				do
				{
					status=GPCTR_Watch((ni_cards[0]).device_number,ND_COUNTER_0,ND_ARMED,
						&entity_value);
/*???debug */
{
	FILE *debug_nidaq;
	...
	if (debug_nidaq=fopen("nidaq.deb","a"))
	{
		fprintf(debug_nidaq,"GPCTR_Watch=%d %d %d\n",status,entity_value,ND_YES);
		fclose(debug_nidaq);
	}
}
				} while (ND_YES==entity_value);
/*???debug */
