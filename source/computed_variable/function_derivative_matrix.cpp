...
//==============================================================================
{
	...
	public:
		...
		Function_variable_derivative_matrix(
			...
			function_derivative_matrix(function_derivative_matrix),column(0),row(0),
		...
		Function_variable_derivative_matrix(
			...
			function_derivative_matrix(function_derivative_matrix),column(column),
			row(row),partial_independent_variables(partial_independent_variables)
		{
			if (function_derivative_matrix)
			{
	...
	public:
		...
		{
			return (function_derivative_matrix);
		}
	...
	private:
		bool equality_atomic(const Function_variable_handle& variable) const
		{
			...
			{
				result=((function_derivative_matrix==
					variable_derivative_matrix->function_derivative_matrix)&&
	...
	private:
		...
		Function_variable_derivative_matrix(
			...
			function_derivative_matrix(variable.function_derivative_matrix),
...
bool is_atomic(Function_variable_derivative_matrix_handle variable)
{
	bool result;
	...
	Scalar& value,const Function_variable_handle variable)
{
	...
	if ((derivative_matrix_variable=boost::dynamic_pointer_cast<
		Function_variable_derivative_matrix,Function_variable>(variable))&&
		is_atomic(derivative_matrix_variable))
	{
...
{
	if (variable&&(0==(variable->partial_independent_variables).size())&&
		(variable->function_derivative_matrix))
	{
		// set up all the possible independent variables
		std::list<Function_variable_handle>::iterator
			independent_variable_iterator;

		for (independent_variable_iterator=(variable->function_derivative_matrix->
			independent_variables).begin();independent_variable_iterator!=(variable->
	...
	if (begin&&variable)
	{
		if (atomic_variable=boost::dynamic_pointer_cast<
			Function_variable_derivative_matrix,Function_variable>(variable->clone()))
		{
			...
			{
				if (atomic_variable->function_derivative_matrix)
				{
					atomic_variable->matrix_reverse_iterator=
						(atomic_variable->function_derivative_matrix->matrices).rend();
					(atomic_variable->matrix_reverse_iterator)--;
					atomic_variable->row=1;
					atomic_variable->column=1;
					matrix_independent_variables_iterator=
...
//==============================================================================
{
	if (variable&&(variable->function_derivative_matrix)&&atomic_variable)
	{
		bool next_matrix;
		...
		if (next_matrix)
		{
			if ((0<(variable->partial_independent_variables).size())||
				(atomic_variable->matrix_reverse_iterator==
				(variable->function_derivative_matrix->matrices).rbegin()))
			{
				// end
				atomic_variable=0;
			}
...
//==============================================================================
{
	if (variable&&(variable->function_derivative_matrix))
	{
		if (atomic_variable)
		{
			...
			if (previous_matrix)
			{
				...
				else
				{
					...
					if (atomic_variable->matrix_reverse_iterator==
						(variable->function_derivative_matrix->matrices).rend())
					{
						// end
						atomic_variable=0;
					}
		...
		else
		{
				...
				variable->clone()))
			{
				...
				else
				{
					...
					{
						atomic_variable->matrix_reverse_iterator=
							(atomic_variable->function_derivative_matrix->matrices).rbegin();
						atomic_variable->row=
							(atomic_variable->matrix_reverse_iterator->size1)();
						atomic_variable->column=
							(atomic_variable->matrix_reverse_iterator->size2)();
...
//==============================================================================
{
	...
	Function_variable_derivative_matrix_handle matrix_variable(
...
#if defined (DEBUG)
	//???debug
	std::cout << "enter Function_derivative_matrix_compose" << std::endl;
#endif // defined (DEBUG)
...
#if defined (DEBUG)
	//???debug
	std::cout << "leave Function_derivative_matrix_compose" << std::endl;
#endif // defined (DEBUG)
		...
		dependent_variable)
	{
			...
			((derivative->matrices).front()).size2()))
		{
				...
				product_orders&&order_g&&sub_order_g&&matrices_g)
			{
				...
				Function_matrix_handle matrix_inverse_handle(0);
				...
				if (matrix_inverse_handle=Function_matrix_handle(new Function_matrix(
					matrix_inverse)))
				{
					if (matrix_inverse_handle=Function_matrix(
...
{
	bool result;
	Function_variable_derivative_matrix_handle atomic_dependent_variable,
		atomic_independent_variable;

	result=false;
	if ((atomic_dependent_variable=boost::dynamic_pointer_cast<
		Function_variable_derivative_matrix,Function_variable>(atomic_variable))&&
		(this==atomic_dependent_variable->function_derivative_matrix)&&
...
{
	bool result;
	Function_variable_derivative_matrix_handle atomic_derivative_matrix_variable;
	Function_variable_value_scalar_handle value_scalar;

	result=false;
	if ((atomic_derivative_matrix_variable=boost::dynamic_pointer_cast<
		Function_variable_derivative_matrix,Function_variable>(atomic_variable))&&
		(this==atomic_derivative_matrix_variable->function_derivative_matrix)&&
		is_atomic(atomic_derivative_matrix_variable)&&atomic_value&&
		(atomic_value->value())&&
