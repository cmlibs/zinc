	...
	else
	{
			...
			Function_variable>(variable->clone()))
		{
			...
			if ((variable->xi)&&(0<variable->number_differentiable()))
			{
				if (0<(variable->indices).size())
				{
					atomic_variable->indices[0]=
						variable->indices[((variable->indices).size())-1];
				}
				else
				{
					atomic_variable->indices[0]=variable->number_differentiable();
				}
				atomic_variable->value_private=Function_variable_value_handle(
					new Function_variable_value_scalar(
					Function_variable_element_xi_set_scalar_function));
...
//==============================================================================
{
	...
	friend int get_previous_node(struct FE_node *node,
		void *get_previous_node_data_void);
	public:
		// constructor
	...
	private:
		...
		Function_variable_iterator component_begin,component_end,component_iterator;
...
//==============================================================================
{
	...
	public:
		...
		Function_variable_nodal_values(
			...
			enum FE_nodal_value_type value_type,int version):value_type(value_type),
		...
		Function_variable_nodal_values(
			const Function_finite_element_handle& function_finite_element,
			const std::string component_name):value_type(FE_NODAL_UNKNOWN),
			version(-1),node((struct FE_node *)NULL),
			function_finite_element(function_finite_element),
			component_number(0)
		{
			if (function_finite_element)
		...
		Function_variable_nodal_values(
			const Function_finite_element_handle& function_finite_element,
			Function_size_type component_number):value_type(FE_NODAL_UNKNOWN),
			version(-1),node((struct FE_node *)NULL),
			function_finite_element(function_finite_element),
			component_number(component_number){};
		Function_variable_nodal_values(
			...
			node(node),function_finite_element(function_finite_element)
		{
			if (node)
			{
				ACCESS(FE_node)(node);
	...
	private:
		...
		Function_variable_nodal_values(
			...
			version(variable_nodal_values.version),node(variable_nodal_values.node),
...
{
	Function_variable_iterator_representation_atomic_nodal_values *representation;
	struct FE_node *previous_node;
}; /* struct Get_previous_node_data */
...
DESCRIPTION :
Finds the node after the one in <get_previous_node_data>.
==============================================================================*/
{
	enum FE_nodal_value_type *nodal_value_type,*nodal_value_types;
	...
	if (node&&(get_previous_node_data=
		...
		(representation=get_previous_node_data->representation)&&
		(variable=representation->variable)&&
		(atomic_variable=representation->atomic_variable)&&
		(function_finite_element=atomic_variable->function_finite_element))
	{
		if (node==get_previous_node_data->representation->atomic_variable->node)
		{
			return_code=1;
		}
		else
		{
			number_of_versions=(function_finite_element->number_of_versions)(
				atomic_variable->component_number,node);
			if (representation->variable->version<number_of_versions)
			{
				number_of_values=1+(function_finite_element->number_of_derivatives)(
					atomic_variable->component_number,node);
				if (nodal_value_types=(function_finite_element->nodal_value_types)(
					atomic_variable->component_number,node))
				{
					value_type_index=0;
					if (FE_NODAL_UNKNOWN==representation->variable->value_type)
					{
					...
					else
					{
						...
						while ((value_type_index<number_of_values)&&
							(*nodal_value_type!=representation->variable->value_type))
						{
							value_type_index++;
							nodal_value_type++;
						}
...
DESCRIPTION :
Finds the node after the one in <get_next_node_data>.
==============================================================================*/
{
	enum FE_nodal_value_type *nodal_value_type,*nodal_value_types;
	int number_of_values,number_of_versions,return_code,value_type_index;
		...
		(function_finite_element=atomic_variable->function_finite_element))
	{
		...
		{
			if (node==get_next_node_data->representation->atomic_variable->node)
			{
...
Function_variable_iterator_representation_atomic_nodal_values::
	Function_variable_iterator_representation_atomic_nodal_values(
	const bool begin,Function_variable_nodal_values_handle variable):
	value_types(0),number_of_values(0),number_of_versions(0),value_type_index(0),
	fe_region(variable->function_finite_element->region()),component_begin(
	((variable->function_finite_element->output)())->begin_atomic()),
	component_end(((variable->function_finite_element->output)())->end_atomic()),
	component_iterator(((variable->function_finite_element->output)())->
	begin_atomic()),atomic_variable(0),variable(variable)
//******************************************************************************
// LAST MODIFIED : 8 April 2004
//
// DESCRIPTION :
	...
	if (begin&&variable&&(variable->function_finite_element))
	{
		...
		if (component_iterator!=component_end)
		{
			if (atomic_variable=Function_variable_nodal_values_handle(
				new Function_variable_nodal_values(
				variable_finite_element->function_finite_element,
				variable_finite_element->component_number,variable->node,
				variable->value_type,variable->version)))
			{
				...
				// save atomic_variable->node for DEACCESSing
				save_node=atomic_variable->node;
				if (atomic_variable->node=FE_region_get_first_FE_node_that(fe_region,
					get_next_node,&get_next_node_data))
				{
					ACCESS(FE_node)(atomic_variable->node);
					if (variable->version<0)
					{
						atomic_variable->version=0;
					}
	...
	if (atomic_variable)
	{
		...
		if (!finished)
		{
			if (variable->version<0)
			{
				...
				{
					atomic_variable->version=number_of_versions-1;
				}
			}
			if (!finished)
			{
				...
				if ((struct FE_node *)NULL==variable->node)
				{
					...
					get_previous_node_data.representation=this;
					...
					{
						// want last so set atomic_variable->node to NULL
...
Function_variable_iterator_representation_atomic_nodal_values::
	...
	component_begin(representation.component_begin),
	component_end(representation.component_end),
	component_iterator(representation.component_iterator),
	atomic_variable(0),variable(representation.variable)
//******************************************************************************
// LAST MODIFIED : 8 April 2004
//
...
//==============================================================================
{
	return ( Function_variable_handle
		(new Function_variable_element_xi(Function_element_xi_handle(this),true,
		false)));
}
...
{
	return (Function_variable_handle
		(new Function_variable_element_xi(Function_element_xi_handle(this))));
}
...
{
	return (Function_variable_handle
		(new Function_variable_element_xi(Function_element_xi_handle(this),false)));
}
...
{
	return (Function_variable_handle
		(new Function_variable_element_xi(Function_element_xi_handle(this),index)));
}
		...
		(atomic_value->value()))
	{
			...
			Function_variable_value>(atomic_value->value())))
		{
			result=value_scalar->set(
				xi_private[(atomic_element_xi_variable->indices)[0]],atomic_value);
		}
	}
...
Function_variable_handle Function_finite_element::nodal_values(int version)
//******************************************************************************
// LAST MODIFIED : 22 March 2004
//
// DESCRIPTION :
...
static FE_value *get_nodal_value_address(struct FE_field *field,
	int component_number,struct FE_node *node,enum FE_nodal_value_type value_type,
	int version)
//******************************************************************************
// LAST MODIFIED : 8 April 2004
//
// DESCRIPTION :
// Used by get_nodal_value and set_nodal_value
//
...
{
	enum FE_nodal_value_type *nodal_value_type,*nodal_value_types;
	FE_value *fe_value,*fe_values,*result;
	int i,number_of_components,number_of_values,number_of_versions;

	result=(FE_value *)NULL;
	if (node&&field&&((1==(number_of_components=
			...
			&fe_values))
		{
			...
			while (i<component_number)
			{
				...
				number_of_values=1+get_FE_node_field_component_number_of_derivatives(
					node,field,i);
				fe_value += number_of_versions*number_of_values;
				i++;
			}
			number_of_versions=get_FE_node_field_component_number_of_versions(node,
			...
			if (version<number_of_versions)
			{
				...
				fe_value += version*number_of_values;
...
bool Function_finite_element::get_nodal_value(
	Function_size_type component_number,struct FE_node *node,
	enum FE_nodal_value_type value_type,int version,Scalar& value)
//******************************************************************************
// LAST MODIFIED : 8 April 2004
//
// DESCRIPTION :
//==============================================================================
{
	...
	if (this)
	{
		if (fe_value=get_nodal_value_address(field_private,(int)component_number-1,
...
//==============================================================================
{
		...
		ALLOCATE(xi_coordinates,FE_value,element_dimension))))
	{
		...
		if (calculate_FE_field(field_private,(atomic_variable_finite_element->
			component_number)-1,node_private,element_private,xi_coordinates,
			(FE_value)time_private,&fe_value))
		{
			Matrix result_matrix(1,1);
...
static int nodal_value_calculate_component_values(
	Function_finite_element_handle function_finite_element,
	struct FE_field *fe_field,int component_number,FE_value fe_time,
	struct FE_element *element,struct FE_node *node,
	enum FE_nodal_value_type nodal_value_type,int version,
						...
						number_of_nodal_values))
					{
						...
						while (return_code&&(i<number_of_element_field_nodes))
						{
							if ((element_field_nodal_value_offsets[i]>=0)&&
								(element_field_number_of_specified_nodal_values[i]>0))
							{
								Function_size_type j,number_of_values=(Function_size_type)
								...
								Function_variable_nodal_values_handle 
									variable(new Function_variable_nodal_values(
									function_finite_element,
									(Function_size_type)(component_number+1),
						...
						while (return_code&&(i<number_of_saved_element_field_nodes))
						{
							Function_variable_nodal_values_handle 
								variable(new Function_variable_nodal_values(
								function_finite_element,element_field_nodes[i]));
...
//==============================================================================
{
		...
		ALLOCATE(xi_coordinates,FE_value,element_dimension))
	{
		...
		std::for_each(atomic_independent_variables.begin(),
			...
			Function_finite_element_check_derivative_functor(
			Function_finite_element_handle(this),zero_derivative,
		...
		else
		{
			FE_value **component_values=(FE_value **)NULL,
				...
				***nodal_value_component_values=(FE_value ***)NULL;
			int **component_monomial_info=(int **)NULL,component_number=
				(int)(atomic_variable_finite_element->component_number)-1,
				number_of_components=(int)((this->number_of_components)()),
				*numbers_of_component_values=(int *)NULL,
				number_of_element_field_nodes=0,number_of_nodal_values=0;
			struct FE_element_field_values *element_field_values;
			struct FE_node **element_field_nodes=(struct FE_node **)NULL;
			Function_size_type number_of_xi=xi_private.size();
			...
			if (element_field_values)
			{
				if (nodal_values_variable)
				{
					result=(0!=nodal_value_calculate_component_values(
						Function_finite_element_handle(this),field_private,
						(int)((atomic_variable_finite_element->component_number)-1),
						(FE_value)time_private,element_private,nodal_values_variable->node,
						nodal_values_variable->value_type,nodal_values_variable->version,
				...
				{
					result=(0!=clear_FE_element_field_values(element_field_values))&&
						(0!=calculate_FE_element_field_values(element_private,field_private,
						(FE_value)time_private,(char)0,element_field_values,
						(struct FE_element *)NULL))&&(0!=extract_component_values(
						element_field_values,number_of_components,component_number,
						&numbers_of_component_values,&component_values))&&
						(0!=extract_component_monomial_info(element_field_values,
						number_of_components,component_number,number_of_xi,
						&numbers_of_component_values,&component_monomial_info,
						&monomial_derivative_values));
				}
						...
						if (derivative_component_values)
						{
							...
							for (i=0;i<derivative_order;i++)
							{
								if (element_xi_variables[i])
								{
									...
									{
										xi_derivative_orders[(element_xi_variables[i]->indices)[0]];
									}
								}
							}
							if (result=calculate_monomial_derivative_values(
								number_of_xi,component_monomial_info[0]+1,
								xi_derivative_orders,xi_array,monomial_derivative_values))
							{
								...
								value_address_2=derivative_component_values[0];
								for (j=numbers_of_component_values[0];j>0;j--)
								{
									component_value +=
...
//==============================================================================
{
		...
		(this==atomic_variable_element_xi->function_finite_element))
	{
		...
		{
			result=value_scalar->set(
				xi_private[(atomic_variable_element_xi->indices)[0]],atomic_value);
		}
	}
