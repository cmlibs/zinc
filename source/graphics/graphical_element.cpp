...
int GT_element_group_process_GT_element_settings_list_contents(
	 struct GT_element_settings *settings,	void *process_temp_data_void)
/*******************************************************************************
...
==============================================================================*/
{
	...
	{
		 if ((process_message = process_temp_data->process_message) && 
				(list_data = process_temp_data->list_data))
		 {
							...
							list_data->settings_string_detail))
				{
					 ...
					 process_message->process_command(INFORMATION_MESSAGE,settings_string);
...
int GT_element_group_process_list_or_write_window_commands(struct GT_element_group *gt_element_group,
...
==============================================================================*/
{
	...
	if (gt_element_group && command_prefix)
	{
		...
		process_message->process_command(INFORMATION_MESSAGE," circle_discretization %d",
			gt_element_group->circle_discretization);
		if (gt_element_group->default_coordinate_field)
		{
				...
				&name))
			{
				...
				process_message->process_command(INFORMATION_MESSAGE," default_coordinate %s",name);
				DEALLOCATE(name);
			}
		}
		process_message->process_command(INFORMATION_MESSAGE,
			...
			gt_element_group->element_discretization.number_in_xi2,
			gt_element_group->element_discretization.number_in_xi3);
		process_message->process_command(INFORMATION_MESSAGE," native_discretization ");
		if (gt_element_group->native_discretization_field)
		{
			...
			{
				make_valid_token(&name);
				process_message->process_command(INFORMATION_MESSAGE,name);
				DEALLOCATE(name);
			}
		}
		else
		{
			process_message->process_command(INFORMATION_MESSAGE,"none");
		}
		if (command_suffix)
		{
		...
		struct GT_element_group_process_temp_data *process_temp_data;
		if (ALLOCATE(process_temp_data,struct GT_element_group_process_temp_data,1))
		{
			 process_temp_data->process_message = process_message;
			 process_temp_data->list_data = &list_data;
			 return_code=FOR_EACH_OBJECT_IN_LIST(GT_element_settings)(
					GT_element_group_process_GT_element_settings_list_contents,(void *)process_temp_data,
					gt_element_group->list_of_settings);
