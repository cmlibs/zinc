...
$scale_factor_set_name = "ScaleFactorSesZ";
$node_set_name = "NodeSesZ";
$interpolation_name = "InterpolatiomZ";
		  ...
		  if (m"(\d)+\)\s+(\w+), \w+,\s+([^,]+), #Components=(\d)+")
		  {
			 ...
			 for ($j = 0 ; $j < $field_number_components{$name} ; $j++)
			 {
				...
				if (m"([^\.\s]+)\.\s+Value index=\s*\d+,\s+#Derivatives=\s*(\d+)\s*\(?([\w/,]*)\)?")
				{
				  ...
				  $field_components_type_names{$name}[$j] = $3;
				}
				elsif (m"([^\.\s]+)\.\s+([^,]+),\s+([^,]+),\s+([^\.]+)\.")
				{
...
#Elements
	 ...
	 elsif (m"#Nodes=\s*(\d+)")
	 {
		...
		$node_set_name++;
		print <<NODESET;
		<parameter_set name="$node_set_name"
                     value_type="integer"
		               number_of_parameters="$node_set_number"/>
...
NODESET
	 ...
	 elsif (m"#Scale factor sets=\s*(\d+)")
	 {
		if ($1 != 1)
		{
		  die ("Only one scale factor set implemented at moment\n$_");
		}
		$_ = <>;
		if (m"#Scale factors=\s*(\d+)")
		{
		  $scale_factor_set_number = $1;
		  $scale_factor_set_name++;

		  print <<SCALESET;
		<parameter_set name="$scale_factor_set_name"
		               number_of_parameters="$scale_factor_set_number"/>
...
#Pre define any new interpolation schemes we haven't seen before
        for ($i = 0 ; $i < $number_of_fields ; $i++)
          ...
          for ($j = 0 ; $j < $field_number_components{$name} ; $j++)
			 {
				if (! $interpolation{$field_components_basis{$name}[$j]})
				{
				  $interpolation_name++;
				  $interpolation{$field_components_basis{$name}[$j]} = 
					 $interpolation_name;
				  print <<INTERPOLATION_1;
		<interpolation name="$interpolation_name"
					      basis="$field_components_basis{$name}[$j]"
					      modification="$field_components_modification{$name}[$j]"
					      type="$field_components_mapping{$name}[$j]">
        <parameters>
INTERPOLATION_1
              @node_set_indices = split(/\s/, $field_components_node_indices{$name}[$j]);
				  @derivative_types = split(/[,\s]/, $field_components_type_names{$name}[$j]);
              @scale_factor_set_indices = split(/\s/, $field_components_scale_factor_indices{$name}[$j]);
              for ($k = 0 ; $k < $field_components_number_of_parameters{$name}[$j] ; $k++)
              {
                if (($k > 0) && ($node_set_indices[$k] eq $node_set_indices[$k-1]))
                {
						 if (defined $derivative_types[$derivative-1])
						 {
							$derivative++;
							$derivative_line = "\n                         parameter_value_type=\"$derivative_types[$derivative-1]\"";
						 }
						 else
						 {
							die "Derivative $derivative type not defined\n@derivative_types";
						 }
                }
                else
                {
           ...
           <value>
              <parameter type="nodal_value_index"
                         parameter_set="$node_set_name"
                         parameter_number="$node_set_indices[$k]"$derivative_line>
              <parameter type="value"
                         parameter_set="$scale_factor_set_name"
                         parameter_number="$scale_factor_set_indices[$k]">
           </value>
INTERPOLATION_2
              }
          ...
          for ($j = 0 ; $j < $field_number_components{$name} ; $j++)
			 {
				...
				<element_field_component ref="$field_components_names{$name}[$j]">
				  <interpolation>$interpolation{$field_components_basis{$name}[$j]}</interpolation>
...
ELEMENT_NODES_1
            $in_element_field = 1;
			 ...
			 <parameter_set ref="$node_set_name">
ELEMENT_NODES_2
          $_ = <>;
			 while ((defined $_) && (! m/(Scale|Nodes)/) &&
				 (! m/$end_match/))
...
ELEMENT_SCALE_1
            $in_element_field = 1;
			 ...
			 <parameter_set ref="$scale_factor_set_name">
ELEMENT_SCALE_2
