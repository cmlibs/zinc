...
TEST(FieldIO, data_and_node_group)
{
    ...
    EXPECT_EQ(OK, result = zinc.root_region.readFile(resourcePath("fieldio/data_and_node_group.exf").c_str()));
...
TEST(FieldIO, changeNodeElementDefinition)
{
    ...
    EXPECT_EQ(RESULT_OK, zinc.root_region.readFile(resourcePath("fieldio/twohermitecubes_noscalefactors.exfile").c_str()));
...
TEST(FieldIO, exEmptyString)
{
    ...
    EXPECT_EQ(RESULT_OK, zinc.root_region.readFile(resourcePath("fieldio/empty_string.exf").c_str()));
...
TEST(FieldIO, ex3NodeTimesequence)
{
    ...
    std::string resource = resourcePath("fieldio/node_time_sequence.exf").c_str();
    ASSERT_EQ(RESULT_OK, zinc.root_region.readFile(resource.c_str()));
    ...
    EXPECT_TRUE(sir.isValid());
    srm = sir.createStreamresourceMemory();//Buffer(buffer, bufferLength);
    EXPECT_TRUE(srm.isValid());
...
TEST(FieldIO, ex3NodeTimesequenceSingleTime)
{
	...
	StreaminformationRegion sir = zinc.root_region.createStreaminformationRegion();
    StreamresourceFile srf = sir.createStreamresourceFile(resourcePath("fieldio/node_time_sequence.exf").c_str());
...
TEST(FieldIO, ex3CompactGroups)
{
    ...
    ASSERT_EQ(RESULT_OK, zinc.root_region.readFile(resourcePath("fieldio/compact_groups.exf").c_str()));
...
TEST(FieldIO, subregionMerge)
{
    ...
    EXPECT_EQ(RESULT_OK, zinc.root_region.readFile(resourcePath("fieldio/subregion_merge.exf").c_str()));
    StreaminformationRegion sir = zinc.root_region.createStreaminformationRegion();
    EXPECT_TRUE(sir.isValid());
    StreamresourceMemory srm = sir.createStreamresourceMemory();
    EXPECT_TRUE(srm.isValid());
