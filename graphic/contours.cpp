...
TEST(cmzn_graphic_contours, create)
{
	...
	EXPECT_EQ(CMISS_OK, cmzn_graphic_contours_destroy(&is));
}
...
TEST(cmzn_graphic_contours, isoscalar_field)
{
	...
	EXPECT_EQ(CMISS_OK, cmzn_graphic_contours_set_isoscalar_field(is, c));
	...
	EXPECT_EQ(CMISS_OK, cmzn_graphic_contours_set_isoscalar_field(is, 0));
	EXPECT_EQ(static_cast<cmzn_field *>(0), cmzn_graphic_contours_get_isoscalar_field(is));
...
TEST(cmzn_graphic_contours, list_isovalues)
{
	...
	EXPECT_EQ(CMISS_OK, cmzn_graphic_contours_set_list_isovalues(is, num, values));
...
TEST(cmzn_graphic_contours, list_isovalues_null)
{
	...
	EXPECT_EQ(CMISS_ERROR_ARGUMENT, cmzn_graphic_contours_set_list_isovalues(0, num, values));
	EXPECT_EQ(CMISS_ERROR_ARGUMENT, cmzn_graphic_contours_set_list_isovalues(is, 5, 0));
	EXPECT_EQ(CMISS_ERROR_ARGUMENT, cmzn_graphic_contours_set_list_isovalues(is, -1, 0));
...
TEST(cmzn_graphic_contours, range_isovalues)
{
	...
	EXPECT_EQ(CMISS_OK, cmzn_graphic_contours_set_range_isovalues(is, 1, 0.3, 0.3));
	EXPECT_EQ(CMISS_OK, cmzn_graphic_contours_set_range_isovalues(is, 1, 0.7, 0.7));
	EXPECT_EQ(CMISS_OK, cmzn_graphic_contours_set_range_isovalues(is, num, first, last));
...
TEST(cmzn_graphic_contours, range_isovalues_null)
{
	...
	const int num = 6;
	double first = 0.1, last = 0.55;
	EXPECT_EQ(CMISS_ERROR_ARGUMENT, cmzn_graphic_contours_set_range_isovalues(0, num, first, last));
	EXPECT_EQ(CMISS_ERROR_ARGUMENT, cmzn_graphic_contours_set_range_isovalues(is, -1, first, last));
	EXPECT_EQ(CMISS_OK, cmzn_graphic_contours_set_range_isovalues(is, 0, first, last));
	EXPECT_EQ(CMISS_OK, cmzn_graphic_contours_set_range_isovalues(is, num, first, last));
